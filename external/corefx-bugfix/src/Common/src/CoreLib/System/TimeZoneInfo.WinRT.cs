...
namespace System
{
	partial class TimeZoneInfo
	{
		...
		{
			bool supportsDst = (timeZoneInformation.TZI.StandardDate.Month != 0);
			...
			if (!supportsDst) {
				...
				return AdjustmentRule.CreateAdjustmentRule (
					...
					noDaylightTransitions: false);  // Bias delta is all what we need from this rule
			}
		...
		private static bool TransitionTimeFromTimeZoneInformation (DYNAMIC_TIME_ZONE_INFORMATION timeZoneInformation, out TransitionTime transitionTime, bool readStartDate)
		{
			...
			if (readStartDate) {
				...
				if (timeZoneInformation.TZI.DaylightDate.Year == 0) {
					transitionTime = TransitionTime.CreateFloatingDateRule (
									 new DateTime (1,	/* year  */
												   1,	/* month */
												   1,	/* day   */
												   timeZoneInformation.TZI.DaylightDate.Hour,
												   timeZoneInformation.TZI.DaylightDate.Minute,
												   timeZoneInformation.TZI.DaylightDate.Second,
												   timeZoneInformation.TZI.DaylightDate.Milliseconds),
									 timeZoneInformation.TZI.DaylightDate.Month,
									 timeZoneInformation.TZI.DaylightDate.Day,   /* Week 1-5 */
									 (DayOfWeek)timeZoneInformation.TZI.DaylightDate.DayOfWeek);
				} else {
				...
				//
				if (timeZoneInformation.TZI.StandardDate.Year == 0) {
					transitionTime = TransitionTime.CreateFloatingDateRule (
									 new DateTime (1,	/* year  */
												   1,	/* month */
												   1,	/* day   */
												   timeZoneInformation.TZI.StandardDate.Hour,
												   timeZoneInformation.TZI.StandardDate.Minute,
												   timeZoneInformation.TZI.StandardDate.Second,
												   timeZoneInformation.TZI.StandardDate.Milliseconds),
									 timeZoneInformation.TZI.StandardDate.Month,
									 timeZoneInformation.TZI.StandardDate.Day,   /* Week 1-5 */
									 (DayOfWeek)timeZoneInformation.TZI.StandardDate.DayOfWeek);
				} else {
					transitionTime = TransitionTime.CreateFixedDateRule (
									 new DateTime (1,	/* year  */
												   1,	/* month */
												   ...
												   timeZoneInformation.TZI.StandardDate.Milliseconds),
									 timeZoneInformation.TZI.StandardDate.Month,
									 timeZoneInformation.TZI.StandardDate.Day);
				}
			}
		...
		internal static TimeZoneInfo TryCreateTimeZone (DYNAMIC_TIME_ZONE_INFORMATION timeZoneInformation)
		{
			...
			return new TimeZoneInfo (
				...
				timeZoneInformation.TZI.GetStandardName(),   // we use the display name as the standared names
				timeZoneInformation.TZI.GetStandardName(),
				timeZoneInformation.TZI.GetDaylightName(),
				zoneRules,
				false);
		}
		...
		private static void GetSystemTimeZonesWinRTFallback (CachedData cachedData)
		{
			...
			foreach (var zone in result)
			{
				...
				cachedData._systemTimeZones.Add(zone.Id, zone);
			}
		}
	}
}
