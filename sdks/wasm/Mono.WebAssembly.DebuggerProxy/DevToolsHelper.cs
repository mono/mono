...
namespace WebAssembly.Net.Debugging {
	...
	internal static class TaskExtensions
	{
		...
		public static async Task<TResult> TimeoutAfter<TResult> (this Task<TResult> task, TimeSpan timeout, string timeoutMessage) {
			...
			// Console.WriteLine ($"-- TimeoutAfter completed");
			if (completedTask == task) {
				// Console.WriteLine ($"\t-- TimeoutAfter, and it was our task.. ");
				timeoutCancellationTokenSource.Cancel();
				return await task;  // Very important in order to propagate exceptions
			} else {
				throw new TimeoutException($"{timeoutMessage}. Timeout: {timeout.TotalSeconds}");
			}
		}
	}
}
