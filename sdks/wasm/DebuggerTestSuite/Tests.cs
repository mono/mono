...
namespace DebuggerTests
{
	...
	public class SourceList : DebuggerTestBase {
		...
		[Fact]
		public async Task TimeoutWaitingForPause () {
			var bp = await SetBreakpoint ("dotnet://debugger-test.dll/debugger-test.cs", 15, 2);
		...
		public async Task TrivalStepping () {
			Console.WriteLine ($"------- In the TEST now --------");

			var bp = await SetBreakpoint ("dotnet://debugger-test.dll/debugger-test.cs", 5, 2);

			Console.WriteLine ($"Sending Eval");
			await EvaluateAndCheck (
				"window.setTimeout(function() { invoke_add(); }, 1);",
				"dotnet://debugger-test.dll/debugger-test.cs", 5, 2,
				"IntAdd",
				wait_for_event_fn: (pause_location) => {
					//make sure we're on the right bp
					Console.WriteLine ($"wait_for_event");
					Assert.Equal (bp.Value ["breakpointId"]?.ToString (), pause_location ["hitBreakpoints"]?[0]?.Value<string> ());
		...
		[Fact]
		public async Task InspectLocalsDuringStepping0 () {
		...
		public async Task InspectLocalsWithStructs (bool use_cfo) {
			...
			var exp = new [] { ("SimpleStructProperty", 2, "Utc"), ("SimpleStructField", 5, "Local") };
		...
		{
			await CheckInspectLocalsAtBreakpointSite (
				"dotnet://debugger-test.dll/debugger-test.cs", 82, 2,
				"OuterMethod",
