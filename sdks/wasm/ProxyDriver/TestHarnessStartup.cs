			...
			app.UseRouter (router => {
				router.MapGet ("launch-chrome-and-connect", async context => {
					...
					try {
						...
						psi.Arguments = $"--headless --disable-gpu --lang=en-US --incognito --no-sandbox --user-data-dir={tempPath} --remote-debugging-port={devToolsUrl.Port} http://{TestHarnessProxy.Endpoint.Authority}/{options.PagePath}";
