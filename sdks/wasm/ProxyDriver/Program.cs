    ...
    public class Program
    {
		public static void Main(string[] args)
		{
			var host = new WebHostBuilder()
		        ...
		        .Build();
			if (args.Length > 0 && args [0].StartsWith("/Debug=",StringComparison.OrdinalIgnoreCase)) {
				var path = args [0].Substring ("/Debug=".Length);
				var elems = path.Split ('/');
				var x = new ChromeDebuggingSession (elems [elems.Length - 1]);
				x.Start ();
			}
			host.Run();
        }
