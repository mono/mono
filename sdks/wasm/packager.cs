...
class Driver {
	...
	const string BINDINGS_MODULE_SUPPORT = "$tool_prefix/support/binding_support.js";
	...
	int Run (string[] args) {
		...
		ninja.WriteLine ($"  command = bash -c '$emcc $emcc_flags {emcc_link_flags} -o $out_js --js-library $tool_prefix/support/library_mono.js --js-library $tool_prefix/support/dotnet_support.js {wasm_core_support_library} $in' {strip_cmd}");
		...
		if (build_wasm) {
			var source_file = Path.GetFullPath (Path.Combine (tool_prefix, "support", "driver.c"));
			...
			var pinvoke_file = Path.GetFullPath (Path.Combine (tool_prefix, "support", "pinvoke-tables-default.h"));
			...
			if (add_binding) {
				var bindings_source_file = Path.GetFullPath (Path.Combine (tool_prefix, "support", "corebindings.c"));
			...
			if (enable_zlib) {
				var zlib_source_file = Path.GetFullPath (Path.Combine (tool_prefix, "support", "zlib-helper.c"));
		...
		if (build_wasm) {
			...
			ninja.WriteLine ($"build $appdir/mono.js $appdir/mono.wasm: emcc-link $builddir/driver.o {zlibhelper} {wasm_core_bindings} {ofiles} {profiler_libs} {runtime_libs} $mono_sdkdir/wasm-runtime-release/lib/libmono-native.a | $tool_prefix/support/library_mono.js $tool_prefix/support/dotnet_support.js {wasm_core_support} $emsdk_env");
