...
namespace WebAssembly.Core {
	public class Float32Array : TypedArray<Float32Array, float> {
		...
		/// <summary>
		/// Copies from an array to a <see cref="T:WebAssembly.Core.Float32Array"/>/>.
		/// </summary>
		/// <returns>The from.</returns>
		/// <param name="source">Source.</param>
		/// <param name="offset">Offset.</param>
		/// <param name="count">Count.</param>
		public unsafe int CopyFrom (float [] source, int offset, int count)
		{
			// target array has to be instantiated.
			...
			fixed (float* pTarget = &source [offset]) {

				var res = Runtime.TypedArrayCopyFrom (JSHandle, (int)pTarget, count, sizeof (float), out int exception);
				if (exception != 0)
					throw new JSException ((string)res);
				return (int)res / sizeof (float);
			}
		...
		/// <summary>
		/// Copies from <see cref="ArraySegment{T}"/> to a <see cref="T:WebAssembly.Core.Float32Array"/>/>.
		/// </summary>
		/// <returns>The number of bytes copied.</returns>
		/// <param name="source">Source.</param>
		public int CopyFrom (ArraySegment<float> source) => CopyFrom (source.Array, source.Offset, source.Count);
