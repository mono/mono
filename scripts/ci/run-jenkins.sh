...
if [[ ${CI_TAGS} == *'win-i386'* ]];
    ...
    ${TESTCMD} --label=make-msvc --timeout=60m --fatal ./msvc/run-msbuild.sh "build" "${PLATFORM}" "release" "/p:PlatformToolset=v142 /p:MONO_TARGET_GC=boehm ${MSBUILD_CXX}"
fi
if [[ ${CI_TAGS} == *'win-'* ]];
    then
    ${TESTCMD} --label=make-msvc-sgen --timeout=60m --fatal ./msvc/run-msbuild.sh "build" "${PLATFORM}" "release" "/p:PlatformToolset=v142 /p:MONO_TARGET_GC=sgen ${MSBUILD_CXX}"
fi
