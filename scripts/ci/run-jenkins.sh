...
if [[ ${CI_TAGS} == *'sdks-android'* ]];
   then
        ...
        if [[ ${CI_TAGS} != *'debug'* ]]; then
            ...
            ${TESTCMD} --label=archive --timeout=180m --fatal make -j ${CI_CPU_COUNT} -C sdks/builds archive-android NINJA= IGNORE_PROVISION_ANDROID=1 IGNORE_PROVISION_MXE=1 CONFIGURATION=debug
        fi
