...
if [[ ${CI_TAGS} == *'product-sdks-android'* ]];
   then
        ...
        ${TESTCMD} --label=runtimes --timeout=120m --fatal make -j4 -C sdks/builds --keep-going package-android-{armeabi,armeabi-v7a,arm64-v8a,x86,x86_64} package-android-host-{Darwin,mxe-Win64} package-llvm-{llvm32,llvm64,llvmwin32,llvmwin64}
        exit 0
fi
