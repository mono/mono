		...
		public void Process (CancellationToken token)
		{
			...
			while (!Reader.BaseStream.EndOfStream) {
				...
				while (Reader.Position < goal) {
					...
					if (ImmediateVisitor != null)
						ev.Accept (ImmediateVisitor);
			...
			case LogEventType.Monitor:
				...
				case LogEventType.MonitorBacktrace:
					return new MonitorEvent {
						// TODO: Event should be a byte field instead of it
						// being encoded completely differently compared to the
						// rest of the format.
						Event = (LogMonitorEvent) ((int) extType >> 4 & 0x3),
						ObjectPointer = ReadObject (),
			...
			case LogEventType.Sample:
				...
				case LogEventType.SampleUnmanagedBinary:
					return new UnmanagedBinaryEvent {
						// TODO: This should be a pointer in the format.
						SegmentPointer = (long) Reader.ReadULeb128 (),
				...
				case LogEventType.SampleCounters: {
					...
					while (true) {
						...
						case LogCounterType.String:
							// TODO: This should be a byte in the format.
							value = Reader.ReadULeb128 () == 1 ? Reader.ReadCString () : null;
							break;
