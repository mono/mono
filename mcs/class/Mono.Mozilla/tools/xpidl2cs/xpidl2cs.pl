...
open FILE, '<', @ARGV or die "Can't open file @ARGV";
...
sub parse_parent {
 ...
 #   print "perl xpidlimport.pl $x.idl\n";
    `perl xpidlimport.pl $x.idl`;
...
sub generate_dependents {
	...
	if (! (-e "$file.cs") && -e "$file.idl") {
	    my $ret = `perl xpidlimport.pl $file.idl`;
	    print "\n$ret";
	}
    }
}
