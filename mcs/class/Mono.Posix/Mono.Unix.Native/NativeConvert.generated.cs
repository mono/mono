...
namespace Mono.Unix.Native {
	...
	public sealed /* static */ partial class NativeConvert
	{
		...
		[DllImport (LIB, EntryPoint="Mono_Posix_FromAddrFamily")]
		private static extern int FromAddrFamily (AddrFamily value, out UInt16 rval);

		public static bool TryFromAddrFamily (AddrFamily value, out UInt16 rval)
		{
			return FromAddrFamily (value, out rval) == 0;
		}

		public static UInt16 FromAddrFamily (AddrFamily value)
		{
			UInt16 rval;
			if (FromAddrFamily (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToAddrFamily")]
		private static extern int ToAddrFamily (UInt16 value, out AddrFamily rval);

		public static bool TryToAddrFamily (UInt16 value, out AddrFamily rval)
		{
			return ToAddrFamily (value, out rval) == 0;
		}

		public static AddrFamily ToAddrFamily (UInt16 value)
		{
			AddrFamily rval;
			if (ToAddrFamily (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}
		...
		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockCtrlMsg")]
		private static extern int FromSockCtrlMsg (SockCtrlMsg value, out Int32 rval);

		public static bool TryFromSockCtrlMsg (SockCtrlMsg value, out Int32 rval)
		{
			return FromSockCtrlMsg (value, out rval) == 0;
		}

		public static Int32 FromSockCtrlMsg (SockCtrlMsg value)
		{
			Int32 rval;
			if (FromSockCtrlMsg (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockCtrlMsg")]
		private static extern int ToSockCtrlMsg (Int32 value, out SockCtrlMsg rval);

		public static bool TryToSockCtrlMsg (Int32 value, out SockCtrlMsg rval)
		{
			return ToSockCtrlMsg (value, out rval) == 0;
		}

		public static SockCtrlMsg ToSockCtrlMsg (Int32 value)
		{
			SockCtrlMsg rval;
			if (ToSockCtrlMsg (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockFlags")]
		private static extern int FromSockFlags (SockFlags value, out Int32 rval);

		public static bool TryFromSockFlags (SockFlags value, out Int32 rval)
		{
			return FromSockFlags (value, out rval) == 0;
		}

		public static Int32 FromSockFlags (SockFlags value)
		{
			Int32 rval;
			if (FromSockFlags (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockFlags")]
		private static extern int ToSockFlags (Int32 value, out SockFlags rval);

		public static bool TryToSockFlags (Int32 value, out SockFlags rval)
		{
			return ToSockFlags (value, out rval) == 0;
		}

		public static SockFlags ToSockFlags (Int32 value)
		{
			SockFlags rval;
			if (ToSockFlags (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockOptName")]
		private static extern int FromSockOptName (SockOptName value, out Int32 rval);

		public static bool TryFromSockOptName (SockOptName value, out Int32 rval)
		{
			return FromSockOptName (value, out rval) == 0;
		}

		public static Int32 FromSockOptName (SockOptName value)
		{
			Int32 rval;
			if (FromSockOptName (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockOptName")]
		private static extern int ToSockOptName (Int32 value, out SockOptName rval);

		public static bool TryToSockOptName (Int32 value, out SockOptName rval)
		{
			return ToSockOptName (value, out rval) == 0;
		}

		public static SockOptName ToSockOptName (Int32 value)
		{
			SockOptName rval;
			if (ToSockOptName (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockProtocol")]
		private static extern int FromSockProtocol (SockProtocol value, out Int32 rval);

		public static bool TryFromSockProtocol (SockProtocol value, out Int32 rval)
		{
			return FromSockProtocol (value, out rval) == 0;
		}

		public static Int32 FromSockProtocol (SockProtocol value)
		{
			Int32 rval;
			if (FromSockProtocol (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockProtocol")]
		private static extern int ToSockProtocol (Int32 value, out SockProtocol rval);

		public static bool TryToSockProtocol (Int32 value, out SockProtocol rval)
		{
			return ToSockProtocol (value, out rval) == 0;
		}

		public static SockProtocol ToSockProtocol (Int32 value)
		{
			SockProtocol rval;
			if (ToSockProtocol (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockType")]
		private static extern int FromSockType (SockType value, out Int32 rval);

		public static bool TryFromSockType (SockType value, out Int32 rval)
		{
			return FromSockType (value, out rval) == 0;
		}

		public static Int32 FromSockType (SockType value)
		{
			Int32 rval;
			if (FromSockType (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockType")]
		private static extern int ToSockType (Int32 value, out SockType rval);

		public static bool TryToSockType (Int32 value, out SockType rval)
		{
			return ToSockType (value, out rval) == 0;
		}

		public static SockType ToSockType (Int32 value)
		{
			SockType rval;
			if (ToSockType (value, out rval) == -1)
				ThrowArgumentException (value);
			return rval;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockaddr")]
		private static extern int FromSockaddr (ref Sockaddr source, IntPtr destination);

		public static bool TryCopy (ref Sockaddr source, IntPtr destination)
		{
			return FromSockaddr (ref source, destination) == 0;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockaddr")]
		private static extern int ToSockaddr (IntPtr source, out Sockaddr destination);

		public static bool TryCopy (IntPtr source, out Sockaddr destination)
		{
			return ToSockaddr (source, out destination) == 0;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockaddrIn")]
		private static extern int FromSockaddrIn (ref SockaddrIn source, IntPtr destination);

		public static bool TryCopy (ref SockaddrIn source, IntPtr destination)
		{
			return FromSockaddrIn (ref source, destination) == 0;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockaddrIn")]
		private static extern int ToSockaddrIn (IntPtr source, out SockaddrIn destination);

		public static bool TryCopy (IntPtr source, out SockaddrIn destination)
		{
			return ToSockaddrIn (source, out destination) == 0;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_FromSockaddrIn6")]
		private static extern int FromSockaddrIn6 (ref SockaddrIn6 source, IntPtr destination);

		public static bool TryCopy (ref SockaddrIn6 source, IntPtr destination)
		{
			return FromSockaddrIn6 (ref source, destination) == 0;
		}

		[DllImport (LIB, EntryPoint="Mono_Posix_ToSockaddrIn6")]
		private static extern int ToSockaddrIn6 (IntPtr source, out SockaddrIn6 destination);

		public static bool TryCopy (IntPtr source, out SockaddrIn6 destination)
		{
			return ToSockaddrIn6 (source, out destination) == 0;
		}
