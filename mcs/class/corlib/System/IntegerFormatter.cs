...
// boundry format percisions (0, 99) and the min / max values
...
//      ParseFormat (format);
//      switch (format type) {
//        case 'G' FormatGeneral(value, percision);
//        case 'R' throw Exception("Invalid blah blah");
//        case 'C' FromatCurrency(value, percision, nfi);
//        etc...
//      }
//   }
// }
...
namespace System {
	...
	internal sealed class IntegerFormatter {
		...
		private static bool ParseFormat (string format, out char specifier,  out int percision)
		{		 		 
			percision = -1;
			specifier = '\0';
			...
			if (length == 2) {
				...
				percision = (chars[1] - '0') * 10 + (chars[2] - '0');
			}
		...
		public static string NumberToString (string format, NumberFormatInfo nfi, byte value)
		{
			...
			if (!ParseFormat (format, out specifier, out percision))
				throw new FormatException ("The specified format is invalid");

			switch(specifier) {
			case 'c': return FormatCurrency (value, percision, nfi);
			case 'C': return FormatCurrency (value, percision, nfi);
			case 'd': return FormatDecimal (value, percision);
			case 'D': return FormatDecimal (value, percision);
			case 'e': return FormatExponential (value, percision, false);
			case 'E': return FormatExponential (value, percision, true);
			case 'f': return FormatFixedPoint (value, percision, nfi);	
			case 'F': return FormatFixedPoint (value, percision, nfi);	
			case 'g': return FormatGeneral (value, percision, nfi, false);
			case 'G': return FormatGeneral (value, percision, nfi, true);
			case 'n': return FormatNumber (value, percision, nfi);
			case 'N': return FormatNumber (value, percision, nfi);
			case 'p': return FormatPercent (value, percision, nfi);
			case 'P': return FormatPercent (value, percision, nfi);
			case 'r': throw new FormatException ("The specified format cannot be used in this instance");
			case 'R': throw new FormatException ("The specified format cannot be used in this instance");
			case 'x': return FormatHexadecimal (value, percision, false);
			case 'X': return FormatHexadecimal (value, percision, true);
			default: 
				throw new FormatException ("The specified format is invalid");
			}
		}		
		...
		private static string FormatCurrency (byte value, int percision, NumberFormatInfo nfi) 
		{
			return FormatCurrency ((uint)value, percision, nfi);
		}

		private static string FormatCurrency (short value, int percision, NumberFormatInfo nfi) 
		{
			return FormatCurrency ((int)value, percision, nfi);			
		}
			
		private static string FormatCurrency (int value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			...
			int padding = (percision >= 0) ? percision : nfi.CurrencyDecimalDigits;	     
		...
		private static string FormatCurrency (long value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			bool negative = (value < 0);
		...
		private static string FormatCurrency (sbyte value, int percision, NumberFormatInfo nfi) 
		{
			return FormatCurrency ((int)value, percision, nfi);
		}

		private static string FormatCurrency (ushort value, int percision, NumberFormatInfo nfi) 
		{
			return FormatCurrency ((uint)value, percision, nfi);			
		}

		private static string FormatCurrency (uint value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
		...
		private static string FormatCurrency (ulong value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
		...
		// represented by using a '-' sign. The percision specifies
		...
		// to left w/ numbers, then if we still have percision left
		// over, pad w/ zeros.
		//

		private static string FormatDecimal (byte value, int percision) 
		{
			return FormatDecimal ((uint)value, percision);
		}

		private static string FormatDecimal (short value, int percision) 
		{
			return FormatDecimal ((int)value, percision);
		}
	
		private static string FormatDecimal (int value, int percision)
		{
			int size = (percision > 0) ? (maxIntLength + percision) : maxIntLength;
			char[] buffy = new char[size];
			int position = size;
			bool negative = (value < 0);
			...
			// if we have percision left over, fill with 0's
			percision -= (size - position); 
			while (percision-- > 0 && position > 1) 
				buffy[--position] = '0';
		...
		private static string FormatDecimal (long value, int percision)
		{
			int size = (percision > 0) ? (maxLongLength + percision) : maxLongLength;
			char[] buffy = new char[size];
			int position = size;
			bool negative = (value < 0);
		...
		private static string FormatDecimal (sbyte value, int percision) 
		{
			return FormatDecimal ((int)value, percision);
		}

		private static string FormatDecimal (ushort value, int percision) 
		{
			return FormatDecimal ((uint)value, percision);
		}

		private static string FormatDecimal (uint value, int percision)
		{
			int size = (percision > 0) ? (maxIntLength + percision) : maxIntLength;
			char[] buffy = new char[size];
			int position = size;
		...
		private static string FormatDecimal (ulong value, int percision)
		{
			int size = (percision > 0) ? (maxLongLength + percision) : maxLongLength;
			char[] buffy = new char[size];
			int position = size;
		...
		// a '-' sign if negative. The percision determines 
		// number of decimal places, if not given go 6 places.
		// If percision > the number of places we need, it
		...
		private static string FormatExponential (byte value, int percision, bool upper) 
		{
			return FormatExponential ((uint)value, percision, upper);
		}

		private static string FormatExponential (short value, int percision, bool upper) 
		{
			return FormatExponential ((int)value, percision, upper);
		}

		private static string FormatExponential (int value, int percision, bool upper)
		{
			bool negative = (value < 0);
			int padding = (percision >= 0) ? percision : 6;
			...
			// or we've passed our specified percision
			while (padding > 0 && position < maxIntLength) {
				buffy[idx++] = tmp[position++];
				padding--;
			}
			
			// if we still have more percision to go, add some
			// zeros
			while (padding > 0) {
				buffy[idx++] = '0';
				padding--;
		...
		private static string FormatExponential (long value, int percision, bool upper)
		{
			bool negative = (value < 0);
			int padding = (percision >= 0) ? percision : 6;
			char[] buffy = new char[(padding + 8)];
		...
		private static string FormatExponential (sbyte value, int percision, bool upper) 
		{
			return FormatExponential ((int)value, percision, upper);
		}

		private static string FormatExponential (ushort value, int percision, bool upper) 
		{
			return FormatExponential ((uint)value, percision, upper);
		}

		private static string FormatExponential (uint value, int percision, bool upper)
		{
			int padding = (percision >= 0) ? percision : 6;
			char[] buffy = new char[(padding + 8)];
			char[] tmp = new char [maxIntLength];
		...
		private static string FormatExponential (ulong value, int percision, bool upper)
		{
			int padding = (percision >= 0) ? percision : 6;
			char[] buffy = new char[(padding + 8)];
			char[] tmp = new char [maxLongLength];
		...
		// from right to left, fill up percision number of 0's,
		// plop a . down, then go for our number. 
		//

		private static string FormatFixedPoint (byte value, int percision, NumberFormatInfo nfi)
		{
			return FormatFixedPoint ((uint)value, percision, nfi);
		}

		private static string FormatFixedPoint (short value, int percision, NumberFormatInfo nfi)
		{
			return FormatFixedPoint ((int)value, percision, nfi);
		}

		private static string FormatFixedPoint (int value, int percision, NumberFormatInfo nfi)
		{
			int padding = (percision >= 0) ? (percision + maxIntLength) : (nfi.NumberDecimalDigits + maxIntLength);
			char[] buffy = new char[padding];
			int position = padding;
			bool negative = (value < 0);
			
			// fill up w/ percision # of 0's
		...
		private static string FormatFixedPoint (long value, int percision, NumberFormatInfo nfi)
		{
			int padding = (percision >= 0) ? (percision + maxLongLength) : (nfi.NumberDecimalDigits + maxLongLength);
			char[] buffy = new char[padding];
			int position = padding;
			bool negative = (value < 0);
		...
		private static string FormatFixedPoint (sbyte value, int percision, NumberFormatInfo nfi)
		{
			return FormatFixedPoint ((int)value, percision, nfi);
		}

		private static string FormatFixedPoint (ushort value, int percision, NumberFormatInfo nfi)
		{
			return FormatFixedPoint ((uint)value, percision, nfi);
		}

		private static string FormatFixedPoint (uint value, int percision, NumberFormatInfo nfi)
		{
			int padding = (percision >= 0) ? (percision + maxIntLength) : (nfi.NumberDecimalDigits + maxIntLength);
			char[] buffy = new char[padding];
			int position = padding;
		...
		private static string FormatFixedPoint (ulong value, int percision, NumberFormatInfo nfi)
		{
			int padding = (percision >= 0) ? (percision + maxLongLength) : (nfi.NumberDecimalDigits + maxLongLength);
			char[] buffy = new char[padding];
			int position = padding;
		...
		// percision is omitted. (This actually doesn't make sense when
		...
		// percision of 10, but since it will just be .00000000, it's
		// discarded.
		//

		private static string FormatGeneral (byte value, int percision, NumberFormatInfo nfi, bool upper) {
			return FormatGeneral ((uint)value, percision, nfi, upper);
		}

		private static string FormatGeneral (short value, int percision, NumberFormatInfo nfi, bool upper) {
			return FormatGeneral ((int)value, percision, nfi, upper);
		}

		private static string FormatGeneral (int value, int percision, NumberFormatInfo nfi, bool upper) 
		{
			bool negative = (value < 0);
			char[] tmp = new char [maxIntLength];
			int exponent = 0;
			...
			// integral values are formatted using fixed point when percision
			...
			if (percision <= 0 || exponent < percision) {
				if (negative) 
					tmp[--position] = '-';
			...
			// else our exponent was > percision, use exponential format
			// percision = number of digits to show. 
			int idx = 0, pow = 1;

			exponent = 0;
			position = maxIntLength;
			
			// Loop through while our number is less than the 10 ^ percision, then
			...
			// finally, make our final buffer, at least percision + 6 for 'E+XX' and '-'
			...
			// for the remaining percisions copy over rounded tmp
			percision--;
			while (percision-- > 0)
				buffy[++position] = tmp[idx++];
		...
		private static string FormatGeneral (long value, int percision, NumberFormatInfo nfi, bool upper) 
		{
			bool negative = (value < 0);
			char[] tmp = new char [maxLongLength];
			int exponent = 0;
		...
		private static string FormatGeneral (sbyte value, int percision, NumberFormatInfo nfi, bool upper) {
			return FormatGeneral ((int)value, percision, nfi, upper);
		}

		private static string FormatGeneral (ushort value, int percision, NumberFormatInfo nfi, bool upper) {
			return FormatGeneral ((uint)value, percision, nfi, upper);
		}

		private static string FormatGeneral (uint value, int percision, NumberFormatInfo nfi, bool upper) 
		{
			char[] tmp = new char [maxIntLength];
			int exponent = 0;
			int position = maxIntLength;
		...
		private static string FormatGeneral (ulong value, int percision, NumberFormatInfo nfi, bool upper) 
		{
			char[] tmp = new char [maxLongLength];
			int exponent = 0;
			int position = maxLongLength;
		...
		// non-zero digit preceeds the decimal separator. The percision
		...
		// number, plus the percision on the end, plus a spot for the negative
		// and a spot for decimal separator.
		//

		private static string FormatNumber (byte value, int percision, NumberFormatInfo nfi) {
			return FormatNumber ((uint)value, percision, nfi);
		}

		private static string FormatNumber (short value, int percision, NumberFormatInfo nfi) {
			return FormatNumber ((int)value, percision, nfi);
		}

		private static string FormatNumber (int value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			...
			int padding = (percision >= 0) ? percision : nfi.NumberDecimalDigits;
		...
		private static string FormatNumber (long value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			char[] groupSeparator = nfi.NumberGroupSeparator.ToCharArray ();
			char[] decimalSeparator = nfi.NumberDecimalSeparator.ToCharArray ();
		...
		private static string FormatNumber (sbyte value, int percision, NumberFormatInfo nfi) {
			return FormatNumber ((int)value, percision, nfi);
		}

		private static string FormatNumber (ushort value, int percision, NumberFormatInfo nfi) {
			return FormatNumber ((uint)value, percision, nfi);
		}

		private static string FormatNumber (uint value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			char[] groupSeparator = nfi.NumberGroupSeparator.ToCharArray ();
			char[] decimalSeparator = nfi.NumberDecimalSeparator.ToCharArray ();
		...
		private static string FormatNumber (ulong value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			char[] groupSeparator = nfi.NumberGroupSeparator.ToCharArray ();
			char[] decimalSeparator = nfi.NumberDecimalSeparator.ToCharArray ();
		...
		private static string FormatPercent (byte value, int percision, NumberFormatInfo nfi) 
		{
			return FormatPercent ((uint)value, percision, nfi);
		}

		private static string FormatPercent (short value, int percision, NumberFormatInfo nfi) 
		{
			return FormatPercent ((int)value, percision, nfi);
		}

		private static string FormatPercent (int value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			bool negative = (value < 0);
			...
			int padding = (percision >= 0) ? percision : nfi.PercentDecimalDigits;	     
			int size = maxIntLength + (groupSeparator.Length * maxIntLength) + padding + 2 + 
			decimalSeparator.Length + symbolLength;	
			char[] buffy = new char[size];
			int position = size;
		...
		private static string FormatPercent (long value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
			bool negative = (value < 0);
		...
		private static string FormatPercent (sbyte value, int percision, NumberFormatInfo nfi) 
		{
			return FormatPercent ((int)value, percision, nfi);
		}

		private static string FormatPercent (ushort value, int percision, NumberFormatInfo nfi) 
		{
			return FormatPercent ((uint)value, percision, nfi);
		}

		private static string FormatPercent (uint value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
		...
		private static string FormatPercent (ulong value, int percision, NumberFormatInfo nfi) 
		{
			int i, j, k;
		...
		// 
		// For strings in base 16. Only valid w/ integers. Percision 
		...
		// percision padding) and 16 when dealing with a long. This
		// is to cut off the loop when dealing with negative values,
		// which will loop forever when you hit -1;
		//

		private static string FormatHexadecimal (byte value, int percision, bool upper)
		{		     
			if (percision < 0) percision = 0;
			int size = maxByteLength + percision;
			char[] buffy = new char[size];
		...
		private static string FormatHexadecimal (short value, int percision, bool upper)
		{
			if (percision < 0) percision = 0;
			int size = maxShortLength + percision;
			char[] buffy = new char[size];
			char[] table = upper ? digitUpperTable : digitLowerTable;
			int position = size;
			short mask = (1 << 4) - 1;
		...
		private static string FormatHexadecimal (int value, int percision, bool upper)
		{
			if (percision < 0) percision = 0;
			int size = maxIntLength + percision;
			char[] buffy = new char[size];
			char[] table = upper ? digitUpperTable : digitLowerTable;
			int position = size;
			int mask = (1 << 4) - 1;
		...
		private static string FormatHexadecimal (long value, int percision, bool upper)
		{
			if (percision < 0) percision = 0;
			int size = maxLongLength + percision;
			char[] buffy = new char[size];
			char[] table = upper ? digitUpperTable : digitLowerTable;
			int position = size;
			long mask = (1 << 4) - 1;
		...
		private static string FormatHexadecimal (sbyte value, int percision, bool upper)
		{
			if (percision < 0) percision = 0;
			int size = maxByteLength + percision;
			char[] buffy = new char[size];
		...
		private static string FormatHexadecimal (ushort value, int percision, bool upper)
		{			
			if (percision < 0) percision = 0;
			int size = maxShortLength + percision;
			char[] buffy = new char[size];
		...
		private static string FormatHexadecimal (uint value, int percision, bool upper)
		{			
			if (percision < 0) percision = 0;
			int size = maxIntLength + percision;
			char[] buffy = new char[size];
		...
		private static string FormatHexadecimal (ulong value, int percision, bool upper)
		{			
			if (percision < 0) percision = 0;
			int size = maxLongLength + percision;
			char[] buffy = new char[size];
