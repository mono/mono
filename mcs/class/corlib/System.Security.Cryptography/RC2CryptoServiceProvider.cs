//
// System.Security.Cryptography.RC2CryptoServiceProvider.cs
//
// Authors: Andrew Birkett (andy@nobugs.org)
//          

using System;

namespace System.Security.Cryptography {

	public sealed class RC2CryptoServiceProvider : RC2 {
		public RC2CryptoServiceProvider()
		{
		}

		public override int EffectiveKeySize {
			get {
				return EffectiveKeySizeValue;
			}
			set {	
				EffectiveKeySizeValue = value;
			}
		}

		public override ICryptoTransform CreateDecryptor(byte[] rgbKey, byte[] iv)
		{
			return new RC2Impl(false, rgbKey, EffectiveKeySizeValue);
		}

		public override ICryptoTransform CreateEncryptor(byte[] rgbKey, byte[] iv)
		{
			return new RC2Impl(true, rgbKey, EffectiveKeySizeValue);
		}

		[MonoTODO]
		public override void GenerateIV()
		{
			IVValue = new byte[BlockSizeValue / 8];
			for (int i=0; i < IVValue.Length; i++) IVValue[i] = 0;
		}

		[MonoTODO]
		public override void GenerateKey()
		{
			KeyValue = new byte[KeySizeValue / 8];
			for (int i=0; i < KeyValue.Length; i++) KeyValue[i] = 0;
		}
	}

	internal class RC2Impl : ICryptoTransform
	{
		private bool encrypt;

		public RC2Impl(bool encrypt, byte[] key, int t1)
		{
			this.encrypt = encrypt;
			R = new UInt32[4];
			KeySetup(key, t1);
		}

		public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
		{
			if (inputCount != 8) {
				throw new ArgumentException("RC2 block size is 64 bits");
			}

			for (int i=0; i < 4; i++) {
				R[i] = (UInt32) (inputBuffer[inputOffset + i*2] + inputBuffer[inputOffset + i*2 + 1] * 256);
			}	
			if (encrypt) {
				j = 0;
				Mix(); Mix(); Mix(); Mix(); Mix();
				Mash();
				Mix(); Mix(); Mix(); Mix(); Mix(); Mix();
				Mash();
				Mix(); Mix(); Mix(); Mix(); Mix();
	
			} else {
				j = 63;
				RMix(); RMix(); RMix(); RMix(); RMix();
				RMash();
				RMix(); RMix(); RMix(); RMix(); RMix(); RMix();
				RMash();
				RMix(); RMix(); RMix(); RMix(); RMix();
			}

			for (int i=0; i < 4; i++) {
				outputBuffer[outputOffset + i*2] = (byte) (R[i] & 0xff);
				outputBuffer[outputOffset + i*2 +1 ] = (byte) ((R[i] >> 8) & 0xff);
			}

			return 8;
		}

		public byte[] TransformFinalBlock (byte[] inputBuffer, int inputOffset, int inputCount)
		{
			byte[] output = new byte[8];
			TransformBlock(inputBuffer, inputOffset, inputCount, output, 0);
			return output;
		}
				
		public bool CanTransformMultipleBlocks {
			get {
				return false;
			}
		}

		public int InputBlockSize {
			get {
				return 8;
			}
		}

		public int OutputBlockSize {
			get {
				return 8;
			}
		}

                void System.IDisposable.Dispose ()
                {
                }
                
		public byte[] pitable = {
			 };
	
		// The expanded key (in bottom 16 bits of each word)
		public UInt32[] K;
	
		// The state (again in bottom 16 bits, although we only
		// clear the top 16 bits if needed)
		private UInt32[] R;
	
		// Key indexer
		private int j;
	
	
		public void KeySetup(byte [] key, int t1)
		{		
			// Expand key into a byte array, then convert to word
			// array since we always access the key in 16bit chunks.
			byte[] L = new byte[128];
	
			int t = key.Length;
			int t8 = (t1 + 7) / 8;
			int tm = 255 % (2 << (8 + t1 - 8*t8 - 1));
	
			for (int i=0; i<t; i++) {
				L[i] = key[i];
			}
	
			for (int i=t; i <= 127; i++) {
				L[i] = (byte) (pitable[ (L[i-1] + L[i-t]) & 0xff]);
			}
	
			L[128-t8] = pitable[L[128-t8] & tm];
	
			for (int i=127-t8; i >= 0; i--) {
				L[i] = pitable[L[i+1] ^ L[i+t8]];
			}
	
			K = new UInt32[64];
			for (int i=0; i < 64; i++) {
				K[i] = (UInt32) (L[i*2] + L[i*2+1] * 256);
			}
		}
	
		private void Mix() 
		{
			R[0] += K[j]   + (R[3] & R[2]) + ((~R[3]) & R[1]);
			R[0] = (R[0] << 1) | (R[0]>>15 & 0x1);
	
			R[1] += K[j+1] + (R[0] & R[3]) + ((~R[0]) & R[2]);
			R[1] = (R[1] << 2) | (R[1]>>14 & 0x3);
	
			R[2] += K[j+2] + (R[1] & R[0]) + ((~R[1]) & R[3]);
			R[2] = (R[2] << 3) | (R[2]>>13 & 0x7);
	
			R[3] += K[j+3] + (R[2] & R[1]) + ((~R[2]) & R[0]);
			R[3] = (R[3] << 5) | (R[3]>>11 & 0x1f);
			j += 4;
		}
	
		private void RMix() 
		{
			R[3] &= 0xffff;
			R[3] = (R[3] >> 5) | ((R[3] & 0x1f) << 11);
			R[3] -= K[j] + (R[2] & R[1]) + ((~R[2]) & R[0]);
	
			R[2] &= 0xffff;
			R[2] = (R[2] >> 3) | ((R[2] & 0x7) << 13);
			R[2] -= K[j-1] + (R[1] & R[0]) + ((~R[1]) & R[3]);
	
			R[1] &= 0xffff;
			R[1] = (R[1] >> 2) | ((R[1] & 0x3) << 14);
			R[1] -= K[j-2] + (R[0] & R[3]) + ((~R[0]) & R[2]);
	
			R[0] &= 0xffff;
			R[0] = (R[0] >> 1) | ((R[0] & 0x1) << 15);
			R[0] -= K[j-3] + (R[3] & R[2]) + ((~R[3]) & R[1]);
	
			j -= 4;
		}
		
		
		private void Mash()
		{
			R[0] += K[R[3] & 63];
			R[1] += K[R[0] & 63];
			R[2] += K[R[1] & 63];
			R[3] += K[R[2] & 63];
		}
	
		private void RMash()
		{
			R[3] -= K[R[2] & 63];
			R[2] -= K[R[1] & 63];
			R[1] -= K[R[0] & 63];
			R[0] -= K[R[3] & 63];
		}
	}
}
