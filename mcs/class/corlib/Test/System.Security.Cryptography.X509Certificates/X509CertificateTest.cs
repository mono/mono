...
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "OU=Secure Server Certification Authority, O=\"RSA Data Security, Inc.\", C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "OU=Server Certification Authority, O=CommerceNet, S=California, C=US", x509.Subject);
#endif
}
...
public void Certificate2 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "OU=Directory Services, O=AT&T, C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "OU=Directory Services, O=AT&T, C=US", x509.Subject);
#endif
}
...
public void Certificate3 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "CN=Brand Name:Product Type, O=CCA - Unique ID, C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "CN=\"0+WKJx+wYEZabSSPVX9kLsnx92s=\", OU=Issuing Financial Institution, O=BrandID, C=US", x509.Subject);
#endif
}
...
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "OU=Secure Server Certification Authority, O=\"RSA Data Security, Inc.\", C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "OU=Secure Server Certification Authority, O=\"RSA Data Security, Inc.\", C=US", x509.Subject);
#endif
}
...
public void Certificate5 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "O=SET Root, C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "O=SET Root, C=US", x509.Subject);
#endif
}
...
public void Certificate8 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "CN=Armed Forces Root, OU=DoD, O=U.S. Government, C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "CN=Armed Forces Root, OU=DoD, O=U.S. Government, C=US", x509.Subject);
#endif
}
...
public void Certificate9 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "OU=VeriSign Class 1 CA - Individual Subscriber, O=\"VeriSign, Inc.\", L=Internet", x509.Issuer);
	Assertion.AssertEquals ("Subject", "E=david@formal.ie, CN=David T. Gray, OU=Digital ID Class 1 - Netscape, OU=\"www.verisign.com/repository/CPS Incorp. by Ref.,LIAB.LTD(c)96\", OU=VeriSign Class 1 CA - Individual Subscriber, O=\"VeriSign, Inc.\", L=Internet", x509.Subject);
#endif
}
...
public void Certificate10 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "OID.1.3.6.1.4.1.42.2.11.2.1=keywitness@keywitness.ca, CN=Keywitness Canada Inc., C=CA", x509.Issuer);
	Assertion.AssertEquals ("Subject", "OID.1.3.6.1.4.1.42.2.11.2.1=keywitness@keywitness.ca, CN=Keywitness Canada Inc., C=CA", x509.Subject);
#endif
}
...
public void Certificate11 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "CN=GTE CyberTrust Root, O=GTE Corporation, C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "CN=GTE CyberTrust Root, O=GTE Corporation, C=US", x509.Subject);
#endif
}
...
public void Certificate12 ()
{
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "CN=Microsoft Code Signing PCA, OU=Copyright (c) 2000 Microsoft Corp., O=Microsoft Corporation, L=Redmond, S=Washington, C=US", x509.Issuer);
	Assertion.AssertEquals ("Subject", "CN=Microsoft Corporation, OU=Copyright (c) 2002 Microsoft Corp., O=Microsoft Corporation, L=Redmond, S=Washington, C=US", x509.Subject);
#endif
}
...
//	Assertion.Assert ("Equals", disk.Equals (clone));
	...
	//Assertion.AssertEquals ("ToString(true)", tsa, x509.ToString (true));
#if NET_2_0
	Assertion.AssertEquals ("Issuer", "C=US, O=\"RSA Data Security, Inc.\", OU=Secure Server Certification Authority", x509.Issuer);
	Assertion.AssertEquals ("Subject", "C=US, S=California, O=CommerceNet, OU=Server Certification Authority", x509.Subject);
#endif
}
