        ...
        private static IdentityReferenceCollection TranslateToSids(IdentityReferenceCollection sourceAccounts, out bool someFailed)
        {
            ...
            try
            {
                ...
                foreach (IdentityReference id in sourceAccounts)
                {
					...
					string name = nta.ToString();

                    if (name.Length * 2 + 2 > ushort.MaxValue)
                    {
                    ...
                    Names[currentName].Buffer = Marshal.StringToCoTaskMemUni(name);

                    Names[currentName].Length = (ushort)(name.Length * 2);
				...
				try {
					...
					foreach (Interop.UNICODE_STRING name in Names)
					{
						Marshal.FreeCoTaskMem(name.Buffer);
					}
				}
                ...
                else if (ReturnCode != 0)
                {
                    ...
                    throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "Interop.LsaLookupNames(2) returned unrecognized status {0}", ReturnCode));
                }
