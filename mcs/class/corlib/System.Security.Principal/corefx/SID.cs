...
nameof(binaryForm));
        ...
        public SecurityIdentifier(WellKnownSidType sidType, SecurityIdentifier domainSid)
        {
            ...
            if ((sidType >= WellKnownSidType.AccountAdministratorSid) && (sidType <= WellKnownSidType.AccountRasAndIasServersSid))
            {
                ...
                else if (ErrorCode != Interop.Errors.ERROR_SUCCESS)
                {
                    ...
                    throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "Win32.GetWindowsAccountDomainSid returned unrecognized error {0}", ErrorCode));
                }
            ...
            {
                throw new ArgumentException("CreateWellKnownSid returned ERROR_INVALID_PARAMETER", "sidType/domainSid");
            }
            else if (Error != Interop.Errors.ERROR_SUCCESS)
            {
                ...
                throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "Win32.CreateWellKnownSid returned unrecognized error {0}", Error));
            }
        ...
        internal SecurityIdentifier GetAccountDomainSid()
        {
            ...
            else if (Error != Interop.Errors.ERROR_SUCCESS)
            {
                ...
                throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "Win32.GetWindowsAccountDomainSid returned unrecognized error {0}", Error));
            }
            return ResultSid;
        }
        ...
        private static IdentityReferenceCollection TranslateToNTAccounts(IdentityReferenceCollection sourceSids, out bool someFailed)
        {
            ...
            try
            {
                ...
                {
                    Debug.Assert(false, string.Format(CultureInfo.InvariantCulture, "Interop.LsaLookupSids returned status {0}", ReturnCode));
                    throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "Interop.LsaLookupSids returned status {0}", ReturnCode));
                }
