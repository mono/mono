...
namespace System.Security.Principal
{
    ...
    public class WindowsIdentity : ClaimsIdentity, IDisposable, ISerializable, IDeserializationCallback
    {
            ...
            : base(null, null, null, ClaimTypes.Name, ClaimTypes.GroupSid)
        {
            ...
            using (SafeLsaHandle lsaHandle = ConnectToLsa())
            {
                ...
                if (!Interop.Advapi32.AllocateLocallyUniqueId(out sourceContext.SourceIdentifier))
                    throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "AllocateLocallyUniqueId returned error {0}", Marshal.GetLastWin32Error()));
        ...
        private static SafeAccessTokenHandle DuplicateAccessToken(IntPtr accessToken)
        {
            ...
            {
                throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "DuplicateHandle returned error {0}", Marshal.GetLastWin32Error()));
            }
            ...
            try
            {
                if (til == TokenImpersonationLevel.None)
                {
                    if (!Interop.Advapi32.DuplicateTokenEx(_safeTokenHandle,
                                                      ...
                                                      ref token))
                        throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "DuplicateTokenEx returned error {0}", Marshal.GetLastWin32Error()));
                }
                ...
                if (!Interop.Advapi32.CheckTokenMembership((til != TokenImpersonationLevel.None ? _safeTokenHandle : token),
                                                      sid.BinaryForm,
                                                      ref isMember))
                    throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "CheckTokenMembership returned error {0}", Marshal.GetLastWin32Error()));
#endif
		...
		{
			return new WindowsImpersonationContext (GetTokenInternal ());
		}
        ...
        private static void RunImpersonatedInternal(SafeAccessTokenHandle token, Action action)
        {
            ...
            if (previousToken == null || previousToken.IsInvalid)
                throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "GetCurrentToken returned error {0}", Marshal.GetLastWin32Error()));
            ...
            ExecutionContext.Run(
                ...
                delegate
                {
                    if (!Interop.Advapi32.RevertToSelf())
                        Environment.FailFast(string.Format(CultureInfo.InvariantCulture, "RevertToSelf returned error {0}", Marshal.GetLastWin32Error()));
        ...
        private static void CurrentImpersonatedTokenChanged(AsyncLocalValueChangedArgs<SafeAccessTokenHandle> args)
        {
            ...
            if (args.CurrentValue != null && !args.CurrentValue.IsInvalid)
            {
                if (!Interop.Advapi32.ImpersonateLoggedOnUser(args.CurrentValue))
                    Environment.FailFast(string.Format(CultureInfo.InvariantCulture, "ImpersonateLoggedOnUser returned error {0}", Marshal.GetLastWin32Error()));
            }
        }
        ...
        private static Exception GetExceptionFromNtStatus(int status)
        {
            ...
            return new SecurityException(string.Format(CultureInfo.InvariantCulture, "NT error {0}", status));
        }
            ...
            switch (dwErrorCode)
            {
                ...
                case Interop.Errors.ERROR_INSUFFICIENT_BUFFER:
                    ...
                    result = Interop.Advapi32.GetTokenInformation(tokenHandle,
                                                             ...
                                                             out dwLength);
                    if (!result)
                        throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "GetTokenInformation returned error {0}", Marshal.GetLastWin32Error()));
                    break;
                ...
                case Interop.Errors.ERROR_INVALID_PARAMETER:
                    ...
                    goto default;
                default:
                    throw new SecurityException(string.Format(CultureInfo.InvariantCulture, "GetTokenInformation returned error {0}", dwErrorCode));
            }
