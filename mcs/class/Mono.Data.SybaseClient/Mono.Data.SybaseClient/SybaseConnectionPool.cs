...
namespace Mono.Data.SybaseClient {
        internal class SybaseConnectionPool : MarshalByRefObject, IList, ICollection, IEnumerable
	{
		...
		public SybaseConnectionPool (string dataSource, int port, int packetSize, int minSize, int maxSize)
		{
		...
		public ITds AllocateConnection ()
		{
			...
			lock (list) {
				for (int i = Count; i < minSize; i += 1)
					Add (new Tds50 (dataSource, port, packetSize));
			}
			...
			if (Count < maxSize) {
				Tds tds = new Tds50 (dataSource, port, packetSize);
				Monitor.Enter (tds);
				Add (tds);
				return tds;
			}
