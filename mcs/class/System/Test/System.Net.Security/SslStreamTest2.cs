...
namespace MonoTests.System.Net.Security
{
	...
	public class SslStreamTest2
	{
		...
		public void HandshakeVerificationFailure_client()
		{
			SetupClientServerConnection();
			var clientStream = new SslStream(m_tcpClient.GetStream(), true, RemoteCertificateValidationCallback_AlwaysFail);
			var serverStream = new SslStream(m_tcpServer.GetStream(), true, RemoteCertificateValidationCallback_AlwaysSucceed);
...
// Note that in contrast to .Net we don't call the verification callback (with a null certificate) if verification is disabled.
		...
		// [TestCase]
		// [ExpectedException(typeof(AuthenticationException))]
		// public void HandshakeVerificationFailure_server()
		// {
		// 	SetupClientServerConnection();
		// 	var clientStream = new SslStream(m_tcpClient.GetStream(), true, RemoteCertificateValidationCallback_AlwaysSucceed);
		// 	var serverStream = new SslStream(m_tcpServer.GetStream(), true, RemoteCertificateValidationCallback_AlwaysFail);
		// 	DoHandshake(clientStream, serverStream);
		// }
Ø
// TODO: Can we support passing on the exception?
		...
		// public void HandshakeVerificationFailure_clientException()
		// {
		// 	SetupClientServerConnection();
		// 	var clientStream = new SslStream(m_tcpClient.GetStream(), true, RemoteCertificateValidationCallback_ThrowException);
		// 	var serverStream = new SslStream(m_tcpServer.GetStream(), true, RemoteCertificateValidationCallback_AlwaysSucceed);
		// 	DoHandshake(clientStream, serverStream);
		// }
					...
					Task.Run(() => {
						try {
							if (clientStream != null)
								clientStream.AuthenticateAsClient("test_host");
						}
						catch {
							m_tcpServer.Close();
							throw;
