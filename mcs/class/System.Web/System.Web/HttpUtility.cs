...
namespace System.Web {
	public sealed class HttpUtility {
		...
		public static string HtmlDecode (string s) 
		{
			...
			foreach (char c in s) 
				...
				case ';' :
					entity += c;
					output.Append (htmlDecoder.Decode (entity));
					entity = String.Empty;
					insideEntity = false;
					break;
				default :
	...
	internal class HtmlDecoder {
		Hashtable entities;

		internal HtmlDecoder ()
		{
		...
		internal string Decode (string s)
		{
			if (s[1] == '#')  {
				if (s[2] == ';')
					return s;
				return ((char) Int32.Parse (s.Substring (2, s.Length - 3))).ToString ();
			}
			else if (entities.ContainsKey (s.Substring (1, s.Length - 2)))
				return entities [s.Substring (1, s.Length - 2)].ToString ();
			else
				return s;
		}
	}
