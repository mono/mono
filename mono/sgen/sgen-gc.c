...
pin_objects_from_nursery_pin_queue (gboolean do_scan_objects, ScanCopyContext ctx)
{
	...
	while (start < end) {
		...
		if (do_scan_objects) {
			...
			GRAY_OBJECT_ENQUEUE (queue, obj_to_pin, desc, FALSE);
...
sgen_pin_object (GCObject *object, SgenGrayQueue *queue)
{
	...
	GRAY_OBJECT_ENQUEUE (queue, object, sgen_obj_get_descriptor_safe (object), FALSE);
}
	...
	for (bigobj = los_object_list; bigobj; bigobj = bigobj->next) {
		...
		if (sgen_find_optimized_pin_queue_area ((char*)bigobj->data, (char*)bigobj->data + sgen_los_object_size (bigobj), &dummy, &dummy)) {
			...
			if (SGEN_OBJECT_HAS_REFERENCES (bigobj->data))
				GRAY_OBJECT_ENQUEUE (gc_thread_gray_queue, bigobj->data, sgen_obj_get_descriptor ((GCObject*)bigobj->data), FALSE);
