...
static void
loongarch64_patch_full (MonoCompile *cfg, MonoDomain *domain, guint8 *code, guint8 *target, int relocation)
{
	switch (relocation) {
...
{
	loongarch64_patch_full(NULL, NULL, code, target, relocation);
}
...
mono_arch_patch_code_new (MonoCompile *cfg, MonoDomain *domain, guint8 *code, MonoJumpInfo *ji, gpointer target)
{
	...
	case MONO_PATCH_INFO_METHOD_JUMP:
		...
		loongarch64_patch_full (cfg, domain, ip, (guint8*)target, MONO_R_LOONGARCH64_B);
		mono_arch_flush_icache (ip, 8);
		break;
	default:
		loongarch64_patch_full (cfg, domain, ip, (guint8*)target, ji->relocation);
		break;
...
mono_arch_emit_prolog (MonoCompile *cfg)
{
	...
	if (loongarch_is_imm12 (-cfg->stack_offset)) {
		...
		loongarch_lu12iw (code, loongarch_r21, cfa_offset >> 12);
	...
	if (cfg->param_area) {
		...
		if (loongarch_is_imm12 (-cfg->param_area)) {
			...
			////NOTE: Assuming the stack size is less than 2G.
			loongarch_lu12iw (code, loongarch_r21, cfg->param_area >> 12);
			loongarch_ori (code, loongarch_r21, loongarch_r21, cfg->param_area & 0xfff);
			loongarch_subd (code, loongarch_sp, loongarch_sp, loongarch_r21);
		}
	}
	...
	if (cfg->method->save_lmf) {
		...
		loongarch_std (code, loongarch_ra, loongarch_fp, cfg->lmf_var->inst_offset + MONO_STRUCT_OFFSET (MonoLMF, pc));
		/* gregs + fp + sp */
		int offsets = cfg->lmf_var->inst_offset + MONO_STRUCT_OFFSET (MonoLMF, gregs);
		g_assert (loongarch_is_imm12 (offsets + 80));
		/* Don't emit unwind info for sp/fp, they are already handled in the prolog */
		loongarch_std (code, loongarch_sp, loongarch_fp, offsets);
		loongarch_std (code, loongarch_fp, loongarch_fp, offsets + 8);
		loongarch_std (code, loongarch_s0, loongarch_fp, offsets + 16);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s0, -cfa_offset + offsets + 16);
		loongarch_std (code, loongarch_s1, loongarch_fp, offsets + 24);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s1, -cfa_offset + offsets + 24);
		loongarch_std (code, loongarch_s2, loongarch_fp, offsets + 32);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s2, -cfa_offset + offsets + 32);
		loongarch_std (code, loongarch_s3, loongarch_fp, offsets + 40);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s3, -cfa_offset + offsets + 40);
		loongarch_std (code, loongarch_s4, loongarch_fp, offsets + 48);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s4, -cfa_offset + offsets + 48);
		loongarch_std (code, loongarch_s5, loongarch_fp, offsets + 56);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s5, -cfa_offset + offsets + 56);
		loongarch_std (code, loongarch_s6, loongarch_fp, offsets + 64);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s6, -cfa_offset + offsets + 64);
		loongarch_std (code, loongarch_s7, loongarch_fp, offsets + 72);
		mono_emit_unwind_op_offset (cfg, code, loongarch_s7, -cfa_offset + offsets + 72);
		loongarch_std (code, loongarch_s8, loongarch_fp, offsets + 80);
		...
		while (saved_regs) {
			if (saved_regs & 1) {
				g_assert (loongarch_is_imm12 (offsets));
				loongarch_std (code, i, loongarch_fp, offsets);
	...
	if (cfg->arch.args_reg) {
		...
		if (loongarch_is_imm12 (cfg->stack_offset)) {
			loongarch_addid (code, cfg->arch.args_reg, loongarch_fp, cfg->stack_offset);
		} else {
			////NOTE: Assuming the args_reg's size is less than 2G.
			loongarch_lu12iw (code, loongarch_r21, cfg->stack_offset >> 12);
