...
#define LDC(n) do { sp->data.i = (n); ++ip; ++sp; } while (0)
		...
		MINT_IN_CASE(MINT_CALL_DELEGATE) {
			...
			child_frame.imethod = del_imethod;
			child_frame.retval = sp;
			sp->data.p = vt_sp;
			sp -= param_count + 1;
			if (is_multicast) {
				// Pass the delegate and the arguments to the delegate invoke
				child_frame.stack_args = sp;
			} else if (child_frame.imethod->param_count == param_count + 1) {
