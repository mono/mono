...
#define LDC(n) do { sp->data.i = (n); ++ip; ++sp; } while (0)
		...
		MINT_IN_CASE(MINT_CALL_VARARG) {
			...
			child_frame->imethod = (InterpMethod*)frame->imethod->data_items [*(const guint16*)(ip + 1)];
	...
	*(fieldtype*)addr = sp[0].data.datamem; \
	ip += 3; \
		...
		MINT_IN_CASE(MINT_LEAVE_S_CHECK) {
			...
			u.leave.ip_offset = frame->ip - frame->imethod->code;
			...
			for (int i = frame->imethod->num_clauses - 1; i >= 0; i--) {
				...
				if (MONO_OFFSET_IN_CLAUSE (clause, u.leave.ip_offset) && (endfinally_ip == NULL || !(MONO_OFFSET_IN_CLAUSE (clause, endfinally_ip - frame->imethod->code)))) {
			...
			if (u.leave.old_list != finally_ips && finally_ips) {
				ip = (const guint16*)finally_ips->data;
				finally_ips = g_slist_remove (finally_ips, ip);
