...
#undef LR_BIAS
			...
			if (imt_arg) {
				...
				gpointer *jte = mono_jumptable_add_entry ();
				/*
				 * Those instructions are never executed, and only
				 * used to direct to place where call->method is stored.
				 */
				code = mono_arm_load_jumptable_entry_addr (code, jte, ARMREG_IP);
				if (call->dynamic_imt_arg)
					jte [0] = NULL;
				else
					jte [0] = (gpointer)call->method;
#else
...
mono_arch_find_imt_method (mgreg_t *regs, guint8 *code)
{
	gpointer method;
#ifdef USE_JUMP_TABLES
	/* Jump table reference is encoded 2 instructions before return address. */
	gpointer *jte = mono_arch_jumptable_entry_from_code (code - 8);
	method = jte [0];
#else
...
static arminstr_t *
load_element_with_regbase (arminstr_t *code, ARMReg dst, ARMReg base, guint32 jti)
{
	if (arm_is_imm12 (jti * 4)) {
		ARM_LDR_IMM (code, dst, base, jti * 4);
	} else {
		code = (arminstr_t*) mono_arm_emit_load_imm ((guint8*) code, dst, jti * 4);
		ARM_LDR_REG_REG (code, dst, base, dst);
	}
	return code;
}
#else
		...
		if (item->is_equals) {
			...
			if (item->check_target_idx || fail_case) {
				if (!item->compare_done || fail_case) {
					...
					code = load_element_with_regbase (code, ARMREG_R1, ARMREG_R2, imt_method_jti);
#else
					imt_method = code;
...
#ifdef USE_JUMP_TABLES
				code = load_element_with_regbase (code, ARMREG_IP, ARMREG_R2, JUMP_CODE_JTI (i));
				ARM_BX_COND (code, ARMREG_IP, ARMCOND_NE);
			...
			if (item->has_target_code) {
				...
				code = load_element_with_regbase (code, ARMREG_R1, ARMREG_R2, target_code_jti);
				/* Restore registers */
			...
			if (fail_case) {
				...
				target_code_jti = TARGET_CODE_JTI (i);
				/* Load target address */
				code = load_element_with_regbase (code, ARMREG_IP, ARMREG_R2, target_code_jti);
				/* Restore registers */
				ARM_POP3 (code, ARMREG_R0, ARMREG_R1, ARMREG_R2);
				/* And branch */
				ARM_BX (code, ARMREG_IP);
...
#ifdef USE_JUMP_TABLES
			code = load_element_with_regbase (code, ARMREG_R1, ARMREG_R2, IMT_METHOD_JTI (i));
			ARM_CMP_REG_REG (code, ARMREG_R0, ARMREG_R1);
			code = load_element_with_regbase (code, ARMREG_IP, ARMREG_R2, JUMP_CODE_JTI (i));
			ARM_BX_COND (code, ARMCOND_GE, ARMREG_IP);
			item->jmp_code = GUINT_TO_POINTER (JUMP_CODE_JTI (i));
#else
			ARM_LDR_IMM (code, ARMREG_R1, ARMREG_PC, 0);
			ARM_CMP_REG_REG (code, ARMREG_R0, ARMREG_R1);
