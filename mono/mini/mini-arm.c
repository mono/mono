...
emit_save_lmf (MonoCompile *cfg, guint8 *code, gint32 lmf_offset)
{
	...
	if (mono_arch_have_fast_tls () && mono_tls_get_tls_offset (TLS_KEY_LMF_ADDR) != -1) {
		...
		mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO,
							 &mono_tls_get_lmf_addr_icall_info);
...
mono_arch_output_basic_block (MonoCompile *cfg, MonoBasicBlock *bb)
{
	...
	//		g_print ("cil code\n");
		...
		case OP_GENERIC_CLASS_INIT: {
			...
			mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO,
								 &mono_generic_class_init_icall_info);
			code = emit_call_seq (cfg, code);
		...
		case OP_THROW: {
			...
			mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO,
					     &mono_arch_throw_exception_icall_info);
			code = emit_call_seq (cfg, code);
			break;
		}
		case OP_RETHROW: {
			if (ins->sreg1 != ARMREG_R0)
				ARM_MOV_REG_REG (code, ARMREG_R0, ins->sreg1);
			mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO,
					     &mono_arch_rethrow_exception_icall_info);
			code = emit_call_seq (cfg, code);
			break;
		}
		...
		case OP_GC_SAFE_POINT: {
			...
			buf [0] = code;
			ARM_B_COND (code, ARMCOND_EQ, 0);
			mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_threads_state_poll_icall_info);
			code = emit_call_seq (cfg, code);
...
mono_arch_emit_prolog (MonoCompile *cfg)
{
	...
	/*
	{
		...
		mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_arm_unaligned_stack_icall_info);
		code = emit_call_seq (cfg, code);
		arm_patch (buf [0], code);
	}
	*/
...
mono_arch_emit_exceptions (MonoCompile *cfg)
{
	...
	for (patch_info = cfg->patch_info; patch_info; patch_info = patch_info->next) {
		...
		case MONO_PATCH_INFO_EXC: {
			...
			patch_info->data.icall_info = &mono_arch_throw_corlib_exception_icall_info;
