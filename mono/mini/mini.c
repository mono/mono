	...
	for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
		...
		for (ins = bb->code; ins; ins = ins->next) {
			...
			for (srcindex = 0; srcindex < 2; ++srcindex) {
				...
				if (((def->opcode == OP_MOVE) || (def->opcode == OP_VMOVE)) && (!defs [def->sreg1] || (def_index [def->sreg1] < def_index [sreg])) && !vreg_is_volatile (cfg, def->sreg1)) {
					int vreg = def->sreg1;
...
#else
			if ((((spec [MONO_INST_DEST] == 'i') || (spec [MONO_INST_DEST] == 'v')) && (ins->dreg >= MONO_MAX_IREGS))) {
#endif
				if (!mono_bitset_test_fast (used, ins->dreg) && !get_vreg_to_inst (cfg, ins->dreg)) {
					/* FIXME: Add more instructions */
					/* OP_ADD_IMM is generated by spill_global_vars */
					if ((ins->opcode == OP_MOVE) || (ins->opcode == OP_VMOVE) || (ins->opcode == OP_ICONST) || (ins->opcode == OP_I8CONST) || (ins->opcode == OP_VZERO) || (ins->opcode == OP_ADD_IMM) || (ins->opcode == OP_R8CONST)) {
