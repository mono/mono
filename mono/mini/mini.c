...
{
	switch (arity) {
#define MONO_ARRAY_NEW(x) case x: return (gpointer)mono_array_new_ ## x;
MONO_ARRAY_NEW_ALL
...
MonoMethodSignature *
mono_get_array_new_signature (int arity)
{
	MonoMethodSignature *res = mono_metadata_signature_alloc (mono_defaults.corlib, arity + 1);
	res->pinvoke = 1;
#ifdef TARGET_WIN32
