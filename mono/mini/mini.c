	...
	for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
		...
		for (ins = bb->code; ins; ins = ins->next) {
			...
			for (srcindex = 0; srcindex < 2; ++srcindex) {
				...
				/* The check makes sure the source of the copy did not change since the copy was made */
				if (MONO_IS_MOVE (def) && (def->opcode != OP_FMOVE) && (!defs [def->sreg1] || (def_index [def->sreg1] < def_index [sreg])) && !vreg_is_volatile (cfg, def->sreg1)) {
					int vreg = def->sreg1;
				...
				else if ((def->opcode == OP_ADD_IMM) && (def->sreg1 == cfg->frame_reg) && (MONO_IS_LOAD_MEMBASE (ins) || MONO_ARCH_IS_OP_MEMBASE (ins->opcode))) {
		...
		for (ins_index = 0; ins_index < nins; ins_index ++) {
			...
			if (((spec [MONO_INST_DEST] == 'i') && (ins->dreg >= MONO_MAX_IREGS)) ||
				...
				if (!mono_bitset_test_fast (used, ins->dreg) && !get_vreg_to_inst (cfg, ins->dreg)) {
					...
					if (MONO_IS_MOVE (ins) || (ins->opcode == OP_ICONST) || (ins->opcode == OP_I8CONST) || (ins->opcode == OP_VZERO) || (ins->opcode == OP_ADD_IMM) || (ins->opcode == OP_R8CONST)) {
