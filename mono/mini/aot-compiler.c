...
emit_globals (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->globals->len; ++i) {
		...
		if (entry == NULL) {
			new_entry->index = hash;
			g_ptr_array_index (table, hash) = (GlobalsTableEntry*)new_entry;
		} else {
			while (entry->next)
				entry = entry->next;
...
compile_methods (MonoAotCompile *acfg)
{
	...
	if (acfg->aot_opts.nthreads > 0) {
		...
		while (i < methods_len) {
			...
			thread = mono_thread_create_internal (mono_domain_get (), compile_thread_main, user_data, MONO_THREAD_CREATE_FLAGS_NONE, error);
			mono_error_assert_ok (error);
