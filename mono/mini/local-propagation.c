	...
	for (bb = cfg->bb_entry; bb; bb = bb->next_bb) {
		...
		for (ins_index = 0; ins_index < nins; ins_index ++) {
			...
			if (((ins->opcode == OP_MOVE) || (ins->opcode == OP_VMOVE)) && (ins_index + 1 < nins)) {
				...
				while ((j < nins - 1) && (def->opcode == OP_NOP)) {
					j ++;
					def = reverse [j];
				}
			...
			if (((spec [MONO_INST_DEST] == 'i') && (ins->dreg >= MONO_MAX_IREGS)) ||
				...
				if (!mono_bitset_test_fast (used, ins->dreg) && 
					...
					if (MONO_IS_MOVE (ins) || (ins->opcode == OP_ICONST) || (ins->opcode == OP_I8CONST) || (ins->opcode == OP_VZERO) || (ins->opcode == OP_ADD_IMM) || (ins->opcode == OP_R8CONST) || (ins->opcode == OP_LADD_IMM) || (ins->opcode == OP_ICONV_TO_U1) || (ins->opcode == OP_ICONV_TO_I1) || (ins->opcode == OP_SEXT_I4) || (ins->opcode == OP_LCONV_TO_U1) || (ins->opcode == OP_ICONV_TO_U2) || (ins->opcode == OP_ICONV_TO_I2) || (ins->opcode == OP_LCONV_TO_I2)) {
