...
mono_create_tls_get (MonoCompile *cfg, MonoTlsKey key)
{
	...
	if (cfg->compile_aot) {
		...
		g_assert (TLS_KEY_THREAD == 0);
		g_assert (TLS_KEY_NUM == 5);
...
{
	mono_check_patch (patch_type, patch_data);
...
mono_ldptr:
		...
		case MONO_CEE_MONO_JIT_ICALL_ADDR: {
			...
			g_assert (callinfo);
			g_assertf (callinfo->name, "%d", (int)token);
			g_assertf (callinfo->func, "%d", (int)token);
