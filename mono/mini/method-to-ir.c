...
mono_find_leave_clauses (MonoCompile *cfg, guchar *ip, guchar *target)
{
	...
	for (i = 0; i < header->num_clauses; ++i) {
		...
		if (MONO_OFFSET_IN_CLAUSE (clause, (ip - header->code)) && 
			...
			MonoLeaveClause *leave = (MonoLeaveClause*)mono_mempool_alloc0 (cfg->mempool, sizeof (MonoLeaveClause));
			leave->index = i;
			leave->clause = clause;
		   ...
		   guint inline_offset, gboolean is_virtual_call)
{
	...
	for (guchar *next_ip = ip; ip < end; ip = next_ip) {
		...
		case MONO_CEE_CALLVIRT: {
			...
			array_rank = 0;
			gboolean virtual_;
			virtual_ = il_op == MONO_CEE_CALLVIRT;
			gboolean pass_imt_from_rgctx;
			pass_imt_from_rgctx = FALSE;
			MonoInst *imt_arg;
			imt_arg = NULL;
			...
			delegate_invoke = FALSE;
			gboolean direct_icall;
			direct_icall = FALSE;
			gboolean constrained_partial_call;
			constrained_partial_call = FALSE;
			gboolean tailcall_calli;
			tailcall_calli = FALSE;
			...
			gboolean called_is_supported_tailcall;
			called_is_supported_tailcall = FALSE;
			MonoMethod *tailcall_method;
			tailcall_method = NULL;
			MonoMethod *tailcall_cmethod;
			tailcall_cmethod = NULL;
			MonoMethodSignature *tailcall_fsig;
			tailcall_fsig = NULL;
			gboolean tailcall_virtual;
			tailcall_virtual = FALSE;
			gboolean tailcall_extra_arg;
			tailcall_extra_arg = FALSE;

			gboolean inst_tailcall;
			inst_tailcall = G_UNLIKELY (debug_tailcall_try_all
							? (next_ip < end && next_ip [0] == CEE_RET)
							: ((ins_flag & MONO_INST_TAILCALL) != 0));
			ins = NULL;
			...
			*/
			...
			gboolean virtual_generic;
			virtual_generic = FALSE;
			gboolean virtual_generic_imt;
			virtual_generic_imt = FALSE;
