...
target_type_is_incompatible (MonoCompile *cfg, MonoType *target, MonoInst *arg)
{
	...
	if (target->byref) {
		...
		if (arg->type == STACK_MP) {
			...
			MonoClass *target_class_lowered = mono_class_from_mono_type (mini_get_underlying_type (m_class_get_byval_arg (mono_class_from_mono_type (target))));
			MonoClass *source_class_lowered = mono_class_from_mono_type (mini_get_underlying_type (m_class_get_byval_arg (arg->klass)));
	...
	case MONO_TYPE_GENERICINST:
		if (mono_type_generic_inst_is_valuetype (simple_type)) {
			...
			if (klass != arg->klass && target_class != arg->klass && target_class != mono_class_from_mono_type (mini_get_underlying_type (m_class_get_byval_arg (arg->klass))))
				return 1;
			return 0;
		} else {
			if (arg->type != STACK_OBJ)
	...
	if (COMPILE_SOFT_FLOAT (cfg)) {
		...
		for (int i = 0; i < sig->param_count + sig->hasthis; ++i) {
			...
			if (!t->byref && t->type == MONO_TYPE_R4) {
				...
				iargs [0] = in;
				conv = mono_emit_jit_icall (cfg, (gconstpointer)mono_fload_r4_arg, iargs);
			    ...
			    MonoInst **args, MonoInst *this_ins, MonoInst *imt_arg, MonoInst *rgctx_arg)
{
	...
	call = mono_emit_call_args (cfg, sig, args, FALSE, virtual_, tailcall, rgctx_arg ? TRUE : FALSE, need_unbox_trampoline, method);
...
emit_method_access_failure (MonoCompile *cfg, MonoMethod *caller, MonoMethod *callee)
{
	...
	mono_emit_jit_icall (cfg, (gconstpointer)mono_throw_method_access, args);
}
...
emit_rgctx_fetch_inline (MonoCompile *cfg, MonoInst *rgctx, MonoJumpInfoRgctxEntry *entry)
{
	...
	if (entry->in_mrgctx)
		call = mono_emit_jit_icall (cfg, (gconstpointer)mono_fill_method_rgctx, args);
	else
		call = mono_emit_jit_icall (cfg, (gconstpointer)mono_fill_class_rgctx, args);
	return call;
#if 0
	/*
...
emit_class_init (MonoCompile *cfg, MonoClass *klass)
{
	...
	if (!COMPILE_LLVM (cfg) && cfg->backend->have_op_generic_class_init) {
		...
		mono_emit_jit_icall (cfg, (gconstpointer)mono_generic_class_init, &vtable_arg);
...
handle_unbox_gsharedvt (MonoCompile *cfg, MonoClass *klass, MonoInst *obj)
{
	...
	/* CASTCLASS */
	obj = mono_emit_jit_icall (cfg, (gconstpointer)mono_object_castclass_unbox, args);
	...
	if (!method->dynamic && !(cfg->opt & MONO_OPT_SHARED)) {
 	...
 	if (cfg->llvm_only) {
		if (virtual_) {
			...
			mono_emit_jit_icall (cfg, (gconstpointer)mono_llvmonly_init_delegate_virtual, args);
		} else {
			mono_emit_jit_icall (cfg, (gconstpointer)mono_llvmonly_init_delegate, &obj);
		}
								   ...
								   gboolean *ref_emit_widen)
{
	...
	if (supported) {
		...
		ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_gsharedvt_constrained_call, args);
		emit_widen = FALSE;
...
emit_llvmonly_virtual_call (MonoCompile *cfg, MonoMethod *cmethod, MonoMethodSignature *fsig, int context_used, MonoInst **sp)
{
	...
	if (!fsig->generic_param_count && !is_iface && !is_gsharedvt) {
		...
		ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_init_vtable_slot, icall_args);
		ins->dreg = slot_reg;
	...
	if ((fsig->generic_param_count || variant_iface || special_array_interface) && !is_gsharedvt) {
		...
		icall_args [2] = emit_get_rgctx_method (cfg, context_used,
												cmethod, MONO_RGCTX_INFO_METHOD);
		if (is_iface)
			ftndesc_ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_resolve_generic_virtual_iface_call, icall_args);
		else
			ftndesc_ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_resolve_generic_virtual_call, icall_args);
		ftndesc_ins->dreg = ftndesc_reg;
		MONO_EMIT_NEW_BRANCH_BLOCK (cfg, OP_BR, end_bb);
	...
	if (is_iface)
		call_target = mono_emit_jit_icall (cfg, (gconstpointer)mono_resolve_iface_call_gsharedvt, icall_args);
	else
		call_target = mono_emit_jit_icall (cfg, (gconstpointer)mono_resolve_vcall_gsharedvt, icall_args);
	...
	for (guchar *next_ip = ip; ip < end; ip = next_ip) {
		...
		case MONO_CEE_CALLI: {
			...
			if (method->dynamic && fsig->pinvoke) {
				...
				args [2] = addr;
				// FIXME tailcall?
				addr = mono_emit_jit_icall (cfg, (gconstpointer)mono_get_native_calli_wrapper, args);
			}
		...
		case MONO_CEE_CALLVIRT: {
			...
			*/
			...
			if (virtual_generic) {
				...
				addr = mono_emit_jit_icall (cfg, (gconstpointer)mono_helper_compile_generic_method, iargs);
			...
			if (array_rank) {
				...
				if (strcmp (cmethod->name, "Set") == 0) { /* array Set */ 
					...
					if (val->type == STACK_OBJ) {
						...
						mono_emit_jit_icall (cfg, (gconstpointer)mono_helper_stelem_ref_check, iargs);
					}
...
calli_end:
			...
			if (cfg->llvm_only && cmethod && method_needs_stack_walk (cfg, cmethod)) {
				 ...
				 */
				mono_emit_jit_icall (cfg, (gconstpointer)mono_dummy_jit_icall, NULL);
			}
		...
		case MONO_CEE_LDSTR:
			if (method->wrapper_type == MONO_WRAPPER_DYNAMIC_METHOD) {
				...
				*sp = mono_emit_jit_icall (cfg, (gconstpointer)mono_string_new_wrapper, iargs);
			} else {
				if (cfg->opt & MONO_OPT_SHARED) {
					MonoInst *iargs [3];
					...
					*sp = mono_emit_jit_icall (cfg, (gconstpointer)ves_icall_mono_ldstr, iargs);
					...
					if (cfg->cbb->out_of_line) {
						...
						if (image == mono_defaults.corlib) {
							...
							*sp = mono_emit_jit_icall (cfg, (gconstpointer)mono_helper_ldstr_mscorlib, iargs);
						} else {
							...
							*sp = mono_emit_jit_icall (cfg, (gconstpointer)mono_helper_ldstr, iargs);
						}
					} 
					else
					if (cfg->compile_aot) {
		...
		case MONO_CEE_NEWOBJ: {
			...
			*/
			...
			if (cfg->cbb->out_of_line && m_class_get_image (cmethod->klass) == mono_defaults.corlib &&
				...
				switch (n) {
				case 0:
					*sp ++ = mono_emit_jit_icall (cfg, (gconstpointer)mono_create_corlib_exception_0, iargs);
					break;
				case 1:
					iargs [1] = sp [0];
					*sp ++ = mono_emit_jit_icall (cfg, (gconstpointer)mono_create_corlib_exception_1, iargs);
					break;
				case 2:
					iargs [1] = sp [0];
					iargs [2] = sp [1];
					*sp ++ = mono_emit_jit_icall (cfg, (gconstpointer)mono_create_corlib_exception_2, iargs);
					break;
				default:
					g_assert_not_reached ();
				}
			...
			if (mini_class_is_system_array (cmethod->klass)) {
				*sp = emit_get_rgctx_method (cfg, context_used,
				...
				if (fsig->param_count == 1)
					alloc = mono_emit_jit_icall (cfg, (gconstpointer)mono_array_new_1, sp);
				else if (fsig->param_count == 2)
					alloc = mono_emit_jit_icall (cfg, (gconstpointer)mono_array_new_2, sp);
				else if (fsig->param_count == 3)
					alloc = mono_emit_jit_icall (cfg, (gconstpointer)mono_array_new_3, sp);
				else if (fsig->param_count == 4)
					alloc = mono_emit_jit_icall (cfg, (gconstpointer)mono_array_new_4, sp);
				else
		...
		case MONO_CEE_STSFLD: {
			...
			*/
			...
			if (is_special_static && ((gsize)addr & 0x80000000) == 0 && thread_ins && !(cfg->opt & MONO_OPT_SHARED) && !context_used) {
				...
				ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_class_static_field_address, iargs);
			} else if (context_used) {
				...
				*/
				...
				if (!addr) {
					...
					ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_get_special_static_data, iargs);
				}
			}
...
field_access_end:
		...
		case MONO_CEE_NEWARR: {
			...
			if (context_used) {
				...
				if (managed_alloc)
					...
					ins = mono_emit_jit_icall (cfg, (gconstpointer)ves_icall_array_new_specific, args);
			} else {
				if (cfg->opt & MONO_OPT_SHARED) {
					...
					ins = mono_emit_jit_icall (cfg, (gconstpointer)ves_icall_array_new, iargs);
				} else {
		...
		case MONO_CEE_CKFINITE: {
			...
			if (cfg->llvm_only) {
				...
				iargs [0] = sp [0];
				*sp++ = mono_emit_jit_icall (cfg, (gconstpointer)mono_ckfinite, iargs);
			} else  {
		...
		case MONO_CEE_LDTOKEN: {
			...
			if ((cfg->opt & MONO_OPT_SHARED) &&
				...
				if (method_context_used) {
					...
					ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_ldtoken_wrapper_generic_shared, iargs);
				} else {
					EMIT_NEW_PCONST (cfg, iargs [2], generic_context);
					ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_ldtoken_wrapper, iargs);
				}
		...
		case MONO_CEE_LEAVE_S: {
			...
			for (i = 0; i < header->num_clauses; ++i) {
				...
				if (MONO_OFFSET_IN_HANDLER (clause, ip - header->code) && (clause->flags == MONO_EXCEPTION_CLAUSE_NONE) && (ip - header->code + ((il_op == MONO_CEE_LEAVE) ? 5 : 2)) <= (clause->handler_offset + clause->handler_len) && method->wrapper_type != MONO_WRAPPER_RUNTIME_INVOKE) {
					...
					*/
					...
					exc_ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_thread_get_undeniable_exception, NULL);
			...
			if ((handlers = mono_find_leave_clauses (cfg, ip, target))) {
				...
				for (tmp = handlers; tmp; tmp = tmp->next) {
					...
					MONO_EMIT_NEW_BRANCH_BLOCK (cfg, OP_PBEQ, dont_throw);
					mono_emit_jit_icall (cfg, (gconstpointer)ves_icall_thread_finish_async_abort, NULL);
...
mono_ldptr:
		...
		case MONO_CEE_MONO_NEWOBJ: {
			...
			*sp++ = mono_emit_jit_icall (cfg, (gconstpointer)ves_icall_object_new, iargs);
			inline_costs += CALL_COST * MIN(10, num_calls++);
			break;
		}
		...
		case MONO_CEE_MONO_JIT_ATTACH: {
			...
			cfg->orig_domain_var = mono_compile_create_var (cfg, mono_get_int_type (), OP_LOCAL);
			...
			if (cfg->compile_aot) {
				...
				ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_jit_thread_attach, args);
			}
			MONO_EMIT_NEW_UNALU (cfg, OP_MOVE, cfg->orig_domain_var->dreg, ins->dreg);
		...
		case MONO_CEE_MONO_JIT_DETACH: {
			...
			mono_emit_jit_icall (cfg, (gconstpointer)mono_jit_set_domain, args);
			break;
		}
		...
		case MONO_CEE_LDFTN: {
			...
			ins = mono_emit_jit_icall (cfg, (gconstpointer)mono_ldftn, &argconst);
			*sp++ = ins;
		...
		case MONO_CEE_LDVIRTFTN: {
			...
			if (context_used)
				*sp++ = mono_emit_jit_icall (cfg, (gconstpointer)mono_ldvirtfn_gshared, args);
			else
				*sp++ = mono_emit_jit_icall (cfg, (gconstpointer)mono_ldvirtfn, args);
