		   ...
		   guint inline_offset, gboolean is_virtual_call)
{
	...
	while (ip < end) {
		...
		{
			int il_op_unused = -1;
			unsigned char *next_ip_unused = ip;
			const int opsize = mono_opcode_value_and_size (&next_ip_unused, end, &il_op_unused);
			CHECK_OPSIZE (opsize);
		}
		...
		case CEE_CALLVIRT: {
			...
			*/
			...
			if ((cfg->opt & MONO_OPT_TAILCALL) && call_opcode == CEE_CALL && cmethod == method && ip + 5 < end && ip [5] == CEE_RET && !vtable_arg) {
