...
do_invoke_method (DebuggerTlsData *tls, Buffer *buf, InvokeData *invoke, guint8 *p, guint8 **endp)
{
	...
	if (invoke->method) {
		...
		mono_runtime_invoke_checked (invoke->method, NULL, invoke->args, &exc, &error);
	...
	res = mono_runtime_invoke_checked (m, m->klass->valuetype ? (gpointer) this_buf : (gpointer) this_arg, args, &exc, &error);
	mono_error_raise_exception (&error); /* FIXME don't raise here */
