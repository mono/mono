...
get_types (gpointer key, gpointer value, gpointer user_data)
{
	...
	for (tmp = domain->domain_assemblies; tmp; tmp = tmp->next) {
		...
		if (ass->image) {
			...
			t = mono_reflection_get_type_checked (ass->image, ass->image, ud->info, ud->ignore_case, &type_resolve, TRUE, probe_type_error);
	...
	case CMD_ASSEMBLY_GET_TYPE: {
		...
		if (!mono_reflection_parse_type_checked (s, &info, error)) {
			...
			if (info.assembly.name)
				NOT_IMPLEMENTED;
			t = mono_reflection_get_type_checked (ass->image, ass->image, &info, ignorecase, &type_resolve, TRUE, error);
			if (!is_ok (error)) {
				mono_error_cleanup (error); /* FIXME don't swallow the error */
				mono_reflection_free_type_info (&info);
				g_free (s);
