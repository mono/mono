...
def parse_mini_ops(target_define):
    ...
    is_enabled = True
    with open (os.path.join (srcdir, "mini-ops.h")) as opcode_file:
            #
        ...
        for line in opcode_file:
            ...
            if m != None:
                ...
                for part in parts:
                    ...
                    if define not in allowed_defines:
                ...
                is_enabled = True
            elif is_enabled and line.startswith ("MINI_OP"):
...
def gen_output(f, opcodes):
    sorted_opcodes = list(opcodes.values ())
        ...
        try:
            if row == 40:
                ...
                row = 0
                 ...
                 row += 1
            f.write ("  {{")
            for j, c in enumerate(op.spec[:MONO_INST_MAX], start=1):
                if c == "":
                    f.write ("   0")
