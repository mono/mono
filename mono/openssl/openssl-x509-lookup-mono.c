...
// random high number
#define MONO_OPENSSL_X509_L_MONO_ADD	36292
...
struct MonoOpenSSLX509LookupMono {
	const void *instance;
...
mono_openssl_x509_lookup_mono_new (void)
{
	...
	mono = OPENSSL_malloc (sizeof (MonoOpenSSLX509LookupMono));
	if (!mono)
		return NULL;

	memset (mono, 0, sizeof (MonoOpenSSLX509LookupMono));
	return mono;
}
...
static int
mono_lookup_install (MonoLookup *lookup, MonoOpenSSLX509LookupMono *mono)
{
	MonoLookupNode *node;
...
static int
mono_lookup_uninstall (MonoOpenSSLX509LookupMono *mono)
{
	MonoLookupNode **ptr;
...
mono_lookup_ctrl (X509_LOOKUP *ctx, int cmd, const char *argp, long argl, char **ret)
{
	...
	MonoOpenSSLX509LookupMono *mono = (MonoOpenSSLX509LookupMono*)argp;

	if (!lookup || cmd != MONO_OPENSSL_X509_L_MONO_ADD)
		return 0;
	if (!mono || mono->lookup)
		return 0;
...
mono_lookup_get_by_subject (X509_LOOKUP *ctx, int type, X509_NAME *name, X509_OBJECT *obj_ret)
{
	...
	name_obj = mono_openssl_x509_name_from_name (name);
	x509 = NULL;
	...
	mono_openssl_x509_name_free (name_obj);
...
{
	if (mono_openssl_x509_lookup_get_type (lookup) != MONO_OPENSSL_X509_LOOKUP_TYPE_MONO)
		return 0;
	return X509_LOOKUP_ctrl (mono_openssl_x509_lookup_peek_lookup (lookup),
