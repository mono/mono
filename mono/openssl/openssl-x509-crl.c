...
mono_openssl_x509_crl_get_by_cert (MonoOpenSSLX509Crl *crl, X509 *x509)
{
	...
	fprintf (stderr, "mono_openssl_x509_crl_get_by_cert: %d - %p\n", ret, revoked);

	if (!ret || !revoked)
		return NULL;

	return mono_openssl_x509_revoked_new (crl, revoked);
}
...
mono_openssl_x509_crl_get_by_serial (MonoOpenSSLX509Crl *crl, void *serial, int len)
{
	...
	fprintf (stderr, "mono_openssl_x509_crl_get_by_serial: %d - %p\n", ret, revoked);
...
{
	return mono_openssl_util_asn1_time_to_ticks (X509_CRL_get_lastUpdate (crl->crl));
}

MONO_API int64_t
mono_openssl_x509_crl_get_next_update (MonoOpenSSLX509Crl *crl)
{
	return mono_openssl_util_asn1_time_to_ticks (X509_CRL_get_nextUpdate (crl->crl));
}
...
{
	return mono_openssl_x509_name_copy (X509_CRL_get_issuer (crl->crl));
}
