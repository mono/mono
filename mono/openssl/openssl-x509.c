...
{
	return mono_openssl_x509_name_copy (X509_get_subject_name (x509));
}

MONO_API MonoOpenSSLX509Name *
mono_openssl_x509_get_issuer_name (X509 *x509)
{
	return mono_openssl_x509_name_copy (X509_get_issuer_name (x509));
}
...
{
	return mono_openssl_util_asn1_time_to_ticks (X509_get_notBefore (x509));
}

MONO_API int64_t
mono_openssl_x509_get_not_after (X509 *x509)
{
	return mono_openssl_util_asn1_time_to_ticks (X509_get_notAfter (x509));
}
...
static int
get_trust_nid (MonoOpenSSLX509Purpose purpose)
{
	switch (purpose) {
		case MONO_OPENSSL_X509_PURPOSE_SSL_CLIENT:
			return NID_client_auth;
		case MONO_OPENSSL_X509_PURPOSE_SSL_SERVER:
...
mono_openssl_x509_add_explicit_trust (X509 *x509, MonoOpenSSLX509TrustKind kind)
{
	...
	if ((kind & MONO_OPENSSL_X509_TRUST_KIND_REJECT_ALL) != 0)
		kind |= MONO_OPENSSL_X509_TRUST_KIND_REJECT_CLIENT | MONO_OPENSSL_X509_TRUST_KIND_REJECT_SERVER;

	if ((kind & MONO_OPENSSL_X509_TRUST_KIND_TRUST_ALL) != 0)
		kind |= MONO_OPENSSL_X509_TRUST_KIND_TRUST_CLIENT | MONO_OPENSSL_X509_TRUST_KIND_TRUST_SERVER;


	if ((kind & MONO_OPENSSL_X509_TRUST_KIND_REJECT_CLIENT) != 0) {
		ret = mono_openssl_x509_add_reject_object (x509, MONO_OPENSSL_X509_PURPOSE_SSL_CLIENT);
		if (!ret)
			return ret;
	}

	if ((kind & MONO_OPENSSL_X509_TRUST_KIND_REJECT_SERVER) != 0) {
		ret = mono_openssl_x509_add_reject_object (x509, MONO_OPENSSL_X509_PURPOSE_SSL_SERVER);
		if (!ret)
			return ret;
	}

	if (ret) {
		// Ignore any MONO_OPENSSL_X509_TRUST_KIND_TRUST_* settings if we added
		// any kind of MONO_OPENSSL_X509_TRUST_KIND_REJECT_* before.
		return ret;
	}

	if ((kind & MONO_OPENSSL_X509_TRUST_KIND_TRUST_CLIENT) != 0) {
		ret = mono_openssl_x509_add_trust_object (x509, MONO_OPENSSL_X509_PURPOSE_SSL_CLIENT);
		if (!ret)
			return ret;
	}

	if ((kind & MONO_OPENSSL_X509_TRUST_KIND_TRUST_SERVER) != 0) {
		ret = mono_openssl_x509_add_trust_object (x509, MONO_OPENSSL_X509_PURPOSE_SSL_SERVER);
		if (!ret)
			return ret;
	}
