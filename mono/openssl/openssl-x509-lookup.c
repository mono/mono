...
get_lookup_method (MonoOpenSSLX509LookupType type)
{
	...
	case MONO_OPENSSL_X509_LOOKUP_TYPE_MONO:
		return mono_openssl_x509_lookup_mono_method ();
	default:
		return NULL;
	}
}
...
mono_openssl_x509_lookup_new (MonoOpenSSLX509Store *store, MonoOpenSSLX509LookupType type)
{
	...
	store_lookup = X509_STORE_add_lookup (mono_openssl_x509_store_peek_store (store), method);
	if (!store_lookup) {
		OPENSSL_free (lookup);
		return NULL;
	}
	...
	lookup->store = mono_openssl_x509_store_up_ref (store);
...
mono_openssl_x509_lookup_free (MonoOpenSSLX509Lookup *lookup)
{
	...
	if (lookup->store) {
		mono_openssl_x509_store_free (lookup->store);
		lookup->store = NULL;
	}
...
mono_openssl_x509_lookup_by_subject (MonoOpenSSLX509Lookup *lookup, MonoOpenSSLX509Name *name)
{
	...
	ret = X509_LOOKUP_by_subject (lookup->lookup, X509_LU_X509, mono_openssl_x509_name_peek_name (name), &obj);
	if (ret != X509_LU_X509) {
