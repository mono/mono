...
do { if (mono_unix_tls_ssl_ctx_is_debug_enabled(ptr)) \
mono_unix_tls_ssl_ctx_debug_printf (ptr, "%s:%d:%s(): " message, __FILE__, __LINE__, \
	__func__); } while (0)

#define debug_printf(ptr,fmt, ...) \
do { if (mono_unix_tls_ssl_ctx_is_debug_enabled(ptr)) \
mono_unix_tls_ssl_ctx_debug_printf (ptr, "%s:%d:%s(): " fmt, __FILE__, __LINE__, \
	__func__, __VA_ARGS__); } while (0)

MONO_API int
mono_unix_tls_ssl_ctx_is_debug_enabled (MonoBtlsSslCtx *ctx)
{
	return ctx->debug_bio != NULL;
}

MONO_API int
mono_unix_tls_ssl_ctx_debug_printf (MonoBtlsSslCtx *ctx, const char *format, ...)
{
	va_list args;
	int ret;
	...
	ret = mono_unix_tls_debug_printf (ctx->debug_bio, format, args);
	va_end (args);
	return ret;
}

MONO_API MonoBtlsSslCtx *
mono_unix_tls_ssl_ctx_new (void)
{
	MonoBtlsSslCtx *ctx;
...
MONO_API MonoBtlsSslCtx *
mono_unix_tls_ssl_ctx_up_ref (MonoBtlsSslCtx *ctx)
{
...
MONO_API int
mono_unix_tls_ssl_ctx_free (MonoBtlsSslCtx *ctx)
{
...
MONO_API SSL_CTX *
mono_unix_tls_ssl_ctx_get_ctx (MonoBtlsSslCtx *ctx)
{
	return ctx->ctx;
}

MONO_API void
mono_unix_tls_ssl_ctx_set_debug_bio (MonoBtlsSslCtx *ctx, BIO *debug_bio)
{
	if (debug_bio)
...
MONO_API void
mono_unix_tls_ssl_ctx_initialize (MonoBtlsSslCtx *ctx, void *instance)
{
...
MONO_API void
mono_unix_tls_ssl_ctx_set_cert_verify_callback (MonoBtlsSslCtx *ptr, MonoBtlsVerifyFunc func, int cert_required)
{
	int mode;
...
MONO_API void
mono_unix_tls_ssl_ctx_set_cert_select_callback (MonoBtlsSslCtx *ptr, MonoBtlsSelectFunc func)
{
...
MONO_API X509_STORE *
mono_unix_tls_ssl_ctx_peek_store (MonoBtlsSslCtx *ctx)
{
	return SSL_CTX_get_cert_store (ctx->ctx);
}

MONO_API void
mono_unix_tls_ssl_ctx_set_min_version (MonoBtlsSslCtx *ctx, int version)
{
	SSL_CTX_set_min_version (ctx->ctx, version);
}

MONO_API void
mono_unix_tls_ssl_ctx_set_max_version (MonoBtlsSslCtx *ctx, int version)
{
	SSL_CTX_set_max_version (ctx->ctx, version);
}

MONO_API int
mono_unix_tls_ssl_ctx_is_cipher_supported (MonoBtlsSslCtx *ctx, uint16_t value)
{
...
MONO_API int
mono_unix_tls_ssl_ctx_set_ciphers (MonoBtlsSslCtx *ctx, int count, const uint16_t *data,
				   int allow_unsupported)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (!cipher) {
			debug_printf (ctx, "mono_unix_tls_ssl_ctx_set_ciphers(): unknown cipher %02x", data [i]);
...
MONO_API int
mono_unix_tls_ssl_ctx_set_verify_param (MonoBtlsSslCtx *ctx, const MonoBtlsX509VerifyParam *param)
{
	return SSL_CTX_set1_param (ctx->ctx, mono_unix_tls_x509_verify_param_peek_param (param));
}

MONO_API int
mono_unix_tls_ssl_ctx_set_client_ca_list (MonoBtlsSslCtx *ctx, int count, int *sizes, const void **data)
{
