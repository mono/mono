...
do { if (mono_uxtls_ssl_ctx_is_debug_enabled(ptr->ctx)) \
mono_uxtls_ssl_ctx_debug_printf (ptr->ctx, "%s:%d:%s(): " message, __FILE__, __LINE__, \
__func__); } while (0)

#define debug_printf(ptr,fmt, ...) \
do { if (mono_uxtls_ssl_ctx_is_debug_enabled(ptr->ctx)) \
mono_uxtls_ssl_ctx_debug_printf (ptr->ctx, "%s:%d:%s(): " fmt, __FILE__, __LINE__, \
...
mono_uxtls_ssl_new (MonoBtlsSslCtx *ctx)
{
	...
	ptr->ctx = mono_uxtls_ssl_ctx_up_ref (ctx);
	ptr->ssl = SSL_new (mono_uxtls_ssl_ctx_get_ctx (ptr->ctx));
...
{
	mono_uxtls_ssl_close (ptr);
	if (ptr->ssl) {
		SSL_free (ptr->ssl);
		ptr->ssl = NULL;
	}
	if (ptr->ctx) {
		mono_uxtls_ssl_ctx_free (ptr->ctx);
		ptr->ctx = NULL;
	}
	free (ptr);
}
...
{
	return SSL_set1_param (ptr->ssl, mono_uxtls_x509_verify_param_peek_param (param));
}
