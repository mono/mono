...
MONO_API MonoBtlsX509StoreCtx *
mono_uxtls_x509_store_ctx_up_ref (MonoBtlsX509StoreCtx *ctx)
{
...
MONO_API int
mono_uxtls_x509_store_ctx_free (MonoBtlsX509StoreCtx *ctx)
{
...
MONO_API int
mono_uxtls_x509_store_ctx_get_error (MonoBtlsX509StoreCtx *ctx, const char **error_string)
{
	int error;
...
MONO_API int
mono_uxtls_x509_store_ctx_get_error_depth (MonoBtlsX509StoreCtx *ctx)
{
	return X509_STORE_CTX_get_error_depth (ctx->ctx);
}

MONO_API MonoBtlsX509Chain *
mono_uxtls_x509_store_ctx_get_chain (MonoBtlsX509StoreCtx *ctx)
{
	STACK_OF(X509) *certs;
...
MONO_API MonoBtlsX509Chain *
mono_uxtls_x509_store_ctx_get_untrusted (MonoBtlsX509StoreCtx *ctx)
{
...
MONO_API int
mono_uxtls_x509_store_ctx_init (MonoBtlsX509StoreCtx *ctx,
...
MONO_API int
mono_uxtls_x509_store_ctx_set_param (MonoBtlsX509StoreCtx *ctx, MonoBtlsX509VerifyParam *param)
{
	return X509_VERIFY_PARAM_set1 (X509_STORE_CTX_get0_param (ctx->ctx), mono_uxtls_x509_verify_param_peek_param (param));
}

MONO_API int
mono_uxtls_x509_store_ctx_verify_cert (MonoBtlsX509StoreCtx *ctx)
{
	return X509_verify_cert (ctx->ctx);
}

MONO_API X509 *
mono_uxtls_x509_store_ctx_get_by_subject (MonoBtlsX509StoreCtx *ctx, MonoBtlsX509Name *name)
{
	X509_OBJECT obj;
	X509 *x509;
	int ret;
...
MONO_API X509 *
mono_uxtls_x509_store_ctx_get_current_cert (MonoBtlsX509StoreCtx *ctx)
{
...
MONO_API X509 *
mono_uxtls_x509_store_ctx_get_current_issuer (MonoBtlsX509StoreCtx *ctx)
{
...
mono_uxtls_x509_store_ctx_get_verify_param (MonoBtlsX509StoreCtx *ctx)
{
...
MONO_API int
mono_uxtls_x509_store_ctx_get_foo (MonoBtlsX509StoreCtx *ctx)
{
	return 0;
}
