...
dump_counter (MonoCounter *counter, FILE *outfile) {
	void *buffer;
	size_t buffer_size = mono_counter_get_size (counter);
	if (buffer_size < 0)
		return;

	buffer = g_malloc0 (buffer_size);
	mono_counters_sample (counter, buffer, buffer_size);

	switch (mono_counter_get_type (counter)) {
	case MONO_COUNTER_INT:
		fprintf (outfile, ENTRY_FMT "%d\n", mono_counter_get_name (counter), *(int*)buffer);
		break;
	case MONO_COUNTER_UINT:
		fprintf (outfile, ENTRY_FMT "%u\n", mono_counter_get_name (counter), *(guint*)buffer);
		break;
	case MONO_COUNTER_LONG:
		if (mono_counter_get_unit (counter) == MONO_COUNTER_TIME)
			fprintf (outfile, ENTRY_FMT "%.2f ms\n", mono_counter_get_name (counter), (double)(*(gint64*)buffer) / 10000.0);
		else
			fprintf (outfile, ENTRY_FMT "%lld\n", mono_counter_get_name (counter), *(gint64*)buffer);
		break;
	case MONO_COUNTER_ULONG:
		fprintf (outfile, ENTRY_FMT "%llu\n", mono_counter_get_name (counter), *(guint64*)buffer);
		break;
	case MONO_COUNTER_WORD:
		fprintf (outfile, ENTRY_FMT "%zd\n", mono_counter_get_name (counter), *(gssize*)buffer);
		break;
	case MONO_COUNTER_DOUBLE:
		fprintf (outfile, ENTRY_FMT "%.4f\n", mono_counter_get_name (counter), *(double*)buffer);
		break;
	case MONO_COUNTER_STRING:
		fprintf (outfile, ENTRY_FMT "%s\n", mono_counter_get_name (counter), (char*)buffer);
		break;
	case MONO_COUNTER_TIME_INTERVAL:
		fprintf (outfile, ENTRY_FMT "%.2f ms\n", mono_counter_get_name (counter), (double)(*(gint64*)buffer) / 1000.0);
		break;
	}
