...
suspend_signal_handler (int _dummy, siginfo_t *info, void *context)
{
	 ...
	 * initialized yet. Also, from a thread_state_init_from_sigctx point of view,
	 * there is no difference between STARTING and DETACHING, as both are characterised
	 * by an unset domain or lmf TLS value.
	 *
