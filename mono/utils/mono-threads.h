...
But extending MonoThreadInfo has the disadvantage that all functions here return type
...
Older version of this were incompatible with C++ typesafe linkage.
The declarations did not match the implementation.
That is fixed by having the function declarations and implementations
...
With C++ other workarounds will be viable.
  ...
  - Inputs can be just derived classes -- this requires changing
    base member access to be implicit instead of currently explicit, i.e.
...
_
SgenThreadInfo *sgen_thread_info;
sgen_thread_info->client_info.info.foo

would become:

struct _SgenClientThreadInfo : MonoThreadInfo {
	...
};

struct _SgenThreadInfo : SgenClientThreadInfo {
	...
};

sgen_thread_info->foo

and "foo" then must be unambiguous.
Then the input wrappers would go away.
The output wrappers would need other treatment.
*/
