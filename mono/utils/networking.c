/*
 * networking.c: Portable network information functions
 *
 * Author:
...
mono_free_address_info (MonoAddressInfo *ai)
{
	...
	while (cur) {
		next = cur->next;
		g_free ((void*)cur->cannonical_name);
		g_free (cur);
		cur = next;
	}
...
mono_socket_address_init (MonoSocketAddress *sa, socklen_t *len, int family, const void *address, int port)
{
	...
	if (family == AF_INET) {
		*len = sizeof (struct sockaddr_in);
		...
		sa->v4.sin_len = sizeof (struct sockaddr_in);
#endif
		...
		*len = sizeof (struct sockaddr_in6);
		...
		sa->v6.sin6_len = sizeof (struct sockaddr_in6);
#endif
	} else {
