...
gc_event (MonoProfiler *profiler, MonoProfilerGCEvent ev, uint32_t generation)
{
	...
	case MONO_GC_EVENT_START:
		...
		/*
		 * We only want to collect roots if they're explicitly enabled, but if
		 * heapshot is enabled, we also only collect roots for GCs where we're
		 * doing an actual heapshot.
		 */
		if (ENABLED (PROFLOG_GC_ROOT_EVENTS) &&
		    (log_config.hs_mode == MONO_PROFILER_HEAPSHOT_NONE || log_profiler.do_heap_walk)) {
			log_profiler.do_root_events = TRUE;
	...
	case MONO_GC_EVENT_POST_STOP_WORLD:
		...
		/*
		 * All heap events are surrounded by a HEAP_START and a HEAP_END event.
		 * Right now, that's the case for GC Moves, GC Roots or heapshots.
		 */
		if (ENABLED (PROFLOG_GC_MOVE_EVENTS) || log_profiler.do_root_events || log_profiler.do_heap_walk) {
	...
	case MONO_GC_EVENT_PRE_START_WORLD:
		...
		if (log_profiler.do_heap_walk) {
			log_profiler.do_heap_walk = FALSE;
			log_profiler.do_root_events = FALSE;
