...
mono_remoting_invoke (MonoObjectHandle real_proxy, MonoMethodMessage *msg, MonoObject **exc, MonoArray **out_args, MonoError *error)
{
	...
	MonoObject *o;
	MonoMethod *im = MONO_HANDLE_GETVAL (real_proxy, vtable->domain->private_invoke_method);
	gpointer pa [4];
	...
	if (!im) {
		...
		MONO_HANDLE_SETVAL (real_proxy, vtable->domain->private_invoke_method, MonoMethod*, im);
	}

	pa [0] = MONO_HANDLE_RAW (real_proxy);
	pa [1] = msg;
	pa [2] = exc;
	pa [3] = out_args;
		     ...
		     MonoObject **exc, MonoArray **out_args, MonoError *error) 
{
	...
	if (target && mono_object_is_transparent_proxy (target)) {
		...
		if (mono_class_is_contextbound (tp->remote_class->proxy_class) && tp->rp->context == (MonoObject *) mono_context_get ()) {
			...
			ret = mono_remoting_invoke (MONO_HANDLE_NEW (MonoObject, (MonoObject *)tp->rp), msg, exc, out_args, error);
			goto exit;
		}
	}
#endif
	...
	MonoVTable *vt;
	vt = mono_class_vtable_checked (domain, object_array_klass, error);
	goto_if_nok (error, return_null);
