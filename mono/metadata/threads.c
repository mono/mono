...
mono_thread_construct_internal (MonoThreadObjectHandle this_obj_handle)
{
	...
	gpointer thread_gchandle = mono_gchandle_from_handle (MONO_HANDLE_CAST (MonoObject, this_obj_handle), TRUE);
 ...
 */
static int
collect_threads (gpointer *thread_handles, int max_threads)
{
...
mono_threads_perform_thread_dump (void)
{
	...
	for (tindex = 0; tindex < nthreads; ++tindex) {
		gpointer handle = thread_array [tindex];
...
{
	MonoAppContext *ctx = (MonoAppContext *) mono_gchandle_get_target_internal (key);
