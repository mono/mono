...
static void
fire_attach_profiler_events (gsize tid)
{
...
mono_thread_attach_full (MonoDomain *domain, gboolean force_attach)
{
	...
	if (!mono_thread_info_current ()->tools_thread)
		...
		fire_attach_profiler_events (MONO_NATIVE_THREAD_ID_TO_UINT (tid));
...
mono_thread_detach_internal (MonoInternalThread *thread)
{
	...
	*/
	...
	if (!mono_thread_info_lookup (MONO_UINT_TO_NATIVE_THREAD_ID (thread->tid))->tools_thread) {
		...
		mono_profiler_gc_root_deregister (((MonoThreadInfo*)thread->thread_info)->stack_start_limit);
	}
