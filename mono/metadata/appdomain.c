...
shadow_copy_create_ini (const char *shadow, const char *filename)
{
	...
	handle = (void **)mono_w32file_create (u16_ini, GENERIC_WRITE, FILE_SHARE_READ|FILE_SHARE_WRITE,
	...
	result = mono_w32file_write (handle, full_path, strlen (full_path), &n, NULL);
	g_free (full_path);
	CloseHandle (handle);
	return result;
}
