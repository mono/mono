...
mono_marshal_init (void)
{
	...
	if (!module_initialized) {
		...
		register_icall (mono_string_from_ansistr, mono_icall_sig_obj_ptr, FALSE);
		...
		register_icall (mono_string_to_ansistr, mono_icall_sig_ptr_obj, FALSE);
		...
		register_icall (mono_string_ansi_to_builder2, mono_icall_sig_object_ptr, FALSE);
...
{
	return mono_string_to_utf8str_impl (s, error);
}
...
{
	return mono_string_utf8_to_builder2_impl (text, error);
}
