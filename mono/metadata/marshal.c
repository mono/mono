...
mono_array_to_lparray_impl (MonoArrayHandle array_handle, MonoError *error)
{
	...
	case MONO_TYPE_OBJECT:
		if (!mono_cominterop_is_interface (klass_element_class)) {
			nativeArraySize = array->max_length;

			struct native_variant *nativeVarArray = g_new (struct native_variant, nativeArraySize);
			MonoMethod *method = mono_get_Marshal_GetNativeVariantForObject ();
