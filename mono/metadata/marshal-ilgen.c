...
emit_native_icall_wrapper_ilgen (MonoMethodBuilder *mb, MonoMethod *method, MonoMethodSignature *csig, gboolean check_exceptions, gboolean aot, MonoMethodPInvoke *piinfo)
{
	...
	if (uses_handles) {
		...
		for (int i = 0; i < csig->param_count; i++) {
			...
			if (get_method_image (method) && ((i == 0 && csig->hasthis) || MONO_TYPE_IS_REFERENCE (csig->params [i]) || mono_type_is_byref_internal (csig->params [i]))) {
				if (!mb->volatile_args) {
					gpointer const mem = mono_image_alloc0 (get_method_image (method), mono_bitset_alloc_size (csig->param_count, 0));
					mb->volatile_args = mono_bitset_mem_new (mem, csig->param_count, 0);
