...
void
mono_profiler_exception_clause_handler (MonoMethod *method, int clause_type, int clause_num, MonoObject *obj)
{
	ProfilerDesc *prof;
	for (prof = prof_list; prof; prof = prof->next) {
		if (prof->events & MONO_PROFILE_EXCEPTIONS) {
			...
			if (prof->exception_clause_cb2)
				prof->exception_clause_cb2 (prof->profiler, method, clause_type, clause_num, obj);
		}
	}
}
