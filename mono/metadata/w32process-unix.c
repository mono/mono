...
ves_icall_System_Diagnostics_Process_ShellExecuteEx_internal (MonoW32ProcessStartInfoHandle proc_start_info, MonoW32ProcessInfo *process_info, MonoError *error)
{
	...
	const gunichar2 *lpFile; lpFile = coop.filename;
	const gunichar2 *lpParameters; lpParameters = coop.arguments;
	const gunichar2 *lpDirectory; lpDirectory = coop.length.working_directory ? coop.working_directory : NULL;
	 ...
	 */
	gunichar2 *args; args = utf16_concat (utf16_quote, lpFile, utf16_quote, lpParameters ? utf16_space : NULL, lpParameters, (const gunichar2 *)NULL);
	if (args == NULL) {
