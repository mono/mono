	...
	case TYPE_ATTRIBUTE_EXPLICIT_LAYOUT: {
		...
		if (layout == TYPE_ATTRIBUTE_EXPLICIT_LAYOUT) {
			for (i = 0; i < top && !mono_class_has_failure (klass); i++) {
				...
				int j = 0;
				for (j = 0; j < size; j++) {
					guint8 type =  (mono_type_is_primitive(field->type) || field->type->type == MONO_TYPE_PTR || (field->type->type == MONO_TYPE_VALUETYPE && m_class_is_enumtype (field->type->data.klass))) ? 1 : 2;
