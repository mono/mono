...
custom_get_instance (SharedCategory *cat, SharedCounter *scounter, char* name)
{
	...
	strcpy (inst->instance_name, name);
...
static char*
append_str (char *dest, const char *source)
{
	const gsize length = strlen (source) + 1;
...
mono_perfcounter_instance_names (const gunichar2 *category, int category_length, MonoError *error)
{
	...
	if (!cat)
		return get_custom_instances (category, error);
	switch (cat->instance_type) {
	case MonoInstance:		return get_mono_instances (error);
	case CPUInstance:		return get_cpu_instances (error);
	case ProcessInstance:		return get_processes_instances (error);
	case NetworkInterfaceInstance:	return get_networkinterface_instances (error);
	case ThreadInstance:
	default:			return mono_array_new_handle (mono_domain_get (), mono_get_string_class (), 0, error);
	}
}
