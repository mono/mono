...
{
	return *((const int*) a) - ((const CultureInfoEntry*) b)->lcid;
}

static int
culture_name_locator (const void *a, const void *b)
{
	return strcmp ((const char*) a, idx2string (((const CultureInfoNameEntry*) b)->name));
}

static int
region_name_locator (const void *a, const void *b)
{
	return strcmp ((const char*) a, idx2string (((const RegionInfoNameEntry*) b)->name));
}
...
create_group_sizes_array (const gint *gs, gint ml)
{
	...
	ret = mono_handle_array_new (mono_domain_get (), mono_get_int32_class (), len);
...
static MONO_HANDLE_TYPE (MonoArray)
create_names_array_idx (const guint16 *names, gsize ml)
{
	MONO_HANDLE_TYPE (MonoArray) ret;
	MonoDomain *domain;
	gsize i;
	...
	ret = mono_handle_array_new (domain, mono_get_string_class (), ml);

	MONO_HANDLE_ARENA_PUSH ();

	for (i = 0; i < ml; i++)
		MONO_HANDLE_ARRAY_SETREF (ret, i, mono_handle_string_new (domain, idx2string (names [i])));
...
static MONO_HANDLE_TYPE (MonoArray)
create_names_array_idx_dynamic (const guint16 *names, gsize ml)
{
	MONO_HANDLE_TYPE (MonoArray) ret;
	MonoDomain *domain;
	gsize i, len = 0;
	...
	ret = mono_handle_array_new (domain, mono_get_string_class (), len);
...
ves_icall_System_Globalization_CalendarData_fill_calendar_data (MonoCalendarData *this_obj, MonoString *name, gint32 calendar_index)
{
	...
	if (name_entry) {
		...
		MONO_HANDLE_SETREF (this_obj_handle, NativeName, mono_handle_string_new (domain, idx2string (ci->nativename)));
		...
		MONO_HANDLE_SETREF (this_obj_handle, MonthDayPattern, mono_handle_string_new (domain, idx2string (dfe->month_day_pattern)));
...
ves_icall_System_Globalization_CultureData_fill_culture_data (MonoCultureData *this_obj, gint32 datetime_index)
{
	...
	MONO_HANDLE_SETREF (this_obj_handle, AMDesignator, mono_handle_string_new (domain, idx2string (dfe->am_designator)));
	MONO_HANDLE_SETREF (this_obj_handle, PMDesignator, mono_handle_string_new (domain, idx2string (dfe->pm_designator)));
	MONO_HANDLE_SETREF (this_obj_handle, TimeSeparator, mono_handle_string_new (domain, idx2string (dfe->time_separator)));
...
ves_icall_System_Globalization_CultureData_fill_number_data (MonoNumberFormatInfo* number, gint32 number_index)
{
	...
	MONO_HANDLE_SETREF (number_handle, currencyDecimalSeparator, mono_handle_string_new (domain, idx2string (nfe->currency_decimal_separator)));
	MONO_HANDLE_SETREF (number_handle, currencyGroupSeparator, mono_handle_string_new (domain, idx2string (nfe->currency_group_separator)));
	...
	MONO_HANDLE_SETREF (number_handle, naNSymbol, mono_handle_string_new (domain, idx2string (nfe->nan_symbol)));
	MONO_HANDLE_SETREF (number_handle, negativeInfinitySymbol, mono_handle_string_new (domain, idx2string (nfe->negative_infinity_symbol)));
	MONO_HANDLE_SETREF (number_handle, negativeSign, mono_handle_string_new (domain, idx2string (nfe->negative_sign)));
	MONO_HANDLE_SET    (number_handle, numberDecimalDigits, nfe->number_decimal_digits);
	MONO_HANDLE_SETREF (number_handle, numberDecimalSeparator, mono_handle_string_new (domain, idx2string (nfe->number_decimal_separator)));
	MONO_HANDLE_SETREF (number_handle, numberGroupSeparator, mono_handle_string_new (domain, idx2string (nfe->number_group_separator)));
	...
	MONO_HANDLE_SETREF (number_handle, percentSymbol, mono_handle_string_new (domain, idx2string (nfe->percent_symbol)));
	MONO_HANDLE_SETREF (number_handle, perMilleSymbol, mono_handle_string_new (domain, idx2string (nfe->per_mille_symbol)));
	MONO_HANDLE_SETREF (number_handle, positiveInfinitySymbol, mono_handle_string_new (domain, idx2string (nfe->positive_infinity_symbol)));
	MONO_HANDLE_SETREF (number_handle, positiveSign, mono_handle_string_new (domain, idx2string (nfe->positive_sign)));
...
construct_culture (MONO_HANDLE_TYPE (MonoCultureInfo) this_obj_handle, const CultureInfoEntry *ci)
{
	...
	MONO_HANDLE_SETREF (this_obj_handle, name, mono_handle_string_new (domain, idx2string (ci->name)));
	MONO_HANDLE_SETREF (this_obj_handle, englishname, mono_handle_string_new (domain, idx2string (ci->englishname)));
	MONO_HANDLE_SETREF (this_obj_handle, nativename, mono_handle_string_new (domain, idx2string (ci->nativename)));
	MONO_HANDLE_SETREF (this_obj_handle, win3lang, mono_handle_string_new (domain, idx2string (ci->win3lang)));
	MONO_HANDLE_SETREF (this_obj_handle, iso3lang, mono_handle_string_new (domain, idx2string (ci->iso3lang)));
	MONO_HANDLE_SETREF (this_obj_handle, iso2lang, mono_handle_string_new (domain, idx2string (ci->iso2lang)));

	// It's null for neutral cultures
	if (ci->territory > 0)
		MONO_HANDLE_SETREF (this_obj_handle, territory, mono_handle_string_new (domain, idx2string (ci->territory)));
...
construct_region (MONO_HANDLE_TYPE (MonoRegionInfo) this_obj_handle, const RegionInfoEntry *ri)
{
	...
	MONO_HANDLE_SETREF (this_obj_handle, iso2name, mono_handle_string_new (domain, idx2string (ri->iso2name)));
	MONO_HANDLE_SETREF (this_obj_handle, iso3name, mono_handle_string_new (domain, idx2string (ri->iso3name)));
	MONO_HANDLE_SETREF (this_obj_handle, win3name, mono_handle_string_new (domain, idx2string (ri->win3name)));
	MONO_HANDLE_SETREF (this_obj_handle, english_name, mono_handle_string_new (domain, idx2string (ri->english_name)));
	MONO_HANDLE_SETREF (this_obj_handle, native_name, mono_handle_string_new (domain, idx2string (ri->native_name)));
	MONO_HANDLE_SETREF (this_obj_handle, currency_symbol, mono_handle_string_new (domain, idx2string (ri->currency_symbol)));
	MONO_HANDLE_SETREF (this_obj_handle, iso_currency_symbol, mono_handle_string_new (domain, idx2string (ri->iso_currency_symbol)));
	MONO_HANDLE_SETREF (this_obj_handle, currency_english_name, mono_handle_string_new (domain, idx2string (ri->currency_english_name)));
	MONO_HANDLE_SETREF (this_obj_handle, currency_native_name, mono_handle_string_new (domain, idx2string (ri->currency_native_name)));
...
ves_icall_System_Globalization_CultureInfo_get_current_locale_name (void)
{
	...
	if (locale == NULL) {
		...
		ret_handle = mono_handle_string_new (mono_domain_get (), locale);
		g_free (locale);
	}
...
string_invariant_compare (MONO_HANDLE_TYPE (MonoString) str1_handle, gint32 off1, gint32 len1, MONO_HANDLE_TYPE (MonoString) str2_handle, gint32 off2, gint32 len2, gint32 options)
{
	...
	ustr1 = mono_handle_string_chars (str1_handle) + off1;
	ustr2 = mono_handle_string_chars (str2_handle) + off2;
...
ves_icall_System_Globalization_CompareInfo_assign_sortkey (MonoCompareInfo *this_obj, MonoSortKey *key, MonoString *source, gint32 options)
{
	...
	source_chars = mono_handle_string_chars (source_handle);
