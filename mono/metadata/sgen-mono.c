...
notify_gc_roots (GCRootReport *report)
{
	if (!report->count)
		return;
	MONO_PROFILER_RAISE (gc_roots, (report->count, (const mono_byte *const *)report->addresses, (const MonoObject *const *)report->objects));
	report->count = 0;
}
...
sgen_client_binary_protocol_collection_begin (int minor_gc_count, int generation)
{
	...
	if (!pseudo_roots_registered) {
		...
		MONO_PROFILER_RAISE (gc_root_register, (SPECIAL_ADDRESS_CRIT_FIN_QUEUE, 1, MONO_ROOT_SOURCE_FINALIZER_QUEUE, NULL, "critical finalizer queue"));
	}
