...
{
	GCRootReport *report = g_cast (gc_data);
	if (*obj)
...
find_pinned_obj (char *addr)
{
	...
	if (idx != pinned_objects.next_slot) {
		if (pinned_objects.data [idx] == addr)
			return g_cast (pinned_objects.data [idx]);
		if (idx == 0)
			return NULL;
	}

	GCObject *obj = g_cast (pinned_objects.data [idx - 1]);
...
{
	ReportHandleStackRoot *ud = g_cast (user_data);
...
report_pin_queue (void)
{
	...
	for (int i = 0; i < pinned_objects.next_slot; ++i) {
		GCObject *obj = g_cast (pinned_objects.data [i]);
...
sgen_client_scan_thread_data (void *start_nursery, void *end_nursery, gboolean precise, ScanCopyContext ctx)
{
	...
	FOREACH_THREAD_EXCLUDE (info, MONO_THREAD_INFO_FLAGS_NO_GC) {
		...
		if (info->client_info.info.handle_stack) {
			...
			*/
			if (precise)
				mono_handle_stack_scan (info->client_info.info.handle_stack, g_cast ((GcScanFunc)ctx.ops->copy_or_mark_object), ctx.queue, precise, TRUE);
			else {
...
{
	return sgen_register_obj_with_weak_fields (g_cast (obj));
}
