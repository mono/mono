...
sgen_client_degraded_allocation (void)
{
	...
	if (mono_atomic_load_i32 (&last_major_gc_warned) < major_gc_count) {
		...
		if (num == 1 || num == 3)
			mono_trace (G_LOG_LEVEL_DEBUG, MONO_TRACE_GC, "Warning: Degraded allocation.  Consider increasing nursery-size if the warning persists.");
		else if (num == 10)
			mono_trace (G_LOG_LEVEL_DEBUG, MONO_TRACE_GC, "Warning: Repeated degraded allocation.  Consider increasing nursery-size.");
